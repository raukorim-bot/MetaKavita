...
#### 1. New Manga & Comic Providers (Priority Integration)
...
- [x] **C4. Kitsu Scraper:** Added Kitsu JSON:API as a reliable global fallback source.
...
- [x] **C6. Bédéthèque Scraper (v1.5.2):** Implemented high-res robust scraping with dynamic CSRF token bypass (`csrf_token_bel`) and smart multi-stage lookup (Album -> Parent Series) for French BDs.

#### 2. Advanced Features
- [x] **C6b. Western Comics & Books Support (B10):** Integrated the Google Books API as a production-ready scraper to allow metadata fetching for Novels, Franco-Belgian BDs, and Western Comics.
- [x] **C7. Plug & Play Modular Scraper Registry (v1.5.2):** Decoupled scraper loading from core files. Standardized metadata providers with the abstract `BaseScraper` class and implemented an automated Registry discovery pattern (`scrapers/__init__.py`). Drop a new scraper in, the app handles the rest.
- [ ] **C8. Playful Statistics Dashboard (B12):** Display fun metrics on the `/stats` page, such as estimated time saved, estimated DeepL Translation costs avoided, and provider usage charts.
- [ ] **C9. Resiliency & Rate-Limiting Control:** Add an automatic exponential backoff retry mechanism to prevent API blocks (429 errors) during very large batches.
...
- [x] **C10. Resilient Multi-API Translator:** Dedicated translation module (`translator.py`) combining Microsoft Azure Translator (Primary F0 tier) and DeepL (Fallback) with automatic switch upon HTTP quota errors.
- [x] **C11. Dynamic Library Routing & Factory Pattern:** Automatic extraction of Kavita library types (`Manga`, `Comic`, `Book`) coupled with a nested Factory pattern to route requests dynamically to category-specific scrapers.
- [x] **C12. Hybrid ComicVine Scraper:** Built a two-step lookup cascade mapping individual album titles (Issues) and resolving parent volumes (Series) with automatic in-memory homonym summary recovery and noisy HTML cleaning.
- [x] **C13. Contextual Title Cleaning:** Tailored query cleaning logic in `scrapers/__init__.py` based on the targeted media type (safely stripping leading zeros in comics while preserving issue numbers).
- [x] **C14. Provider Purge (Nautiljon):** Completely removed Nautiljon from the default routing map because their admins are terrified of their processors heating up, resulting in abusive Cloudflare IP bans. Switched to robust, open APIs instead.
...
#### 1. Nouveaux Scrapers Manga & BD (Priorité d'intégration)
...
- [x] **C6. Scraper Bédéthèque (v1.5.2) :** Scraping BeautifulSoup4 robuste avec contournement par empreinte TLS (`curl_cffi`) du jeton de sécurité CSRF (`csrf_token_bel`) et chaînage intelligent des fiches (Album -> Série).

#### 2. Fonctionnalités Avancées
- [x] **C7. Support des BD Occidentales & Romans (B10) :** Intégration de l'API Google Books en scraper de production pour enrichir les romans, les BD franco-belges et comics américains (compatible catégories Comic et Book).
- [x] **C8. Registre Modulaire Plug & Play (v1.5.2) :** Découplage total des scrapers et du code noyau. Standardisation des fiches via la classe abstraite `BaseScraper` et implémentation du registre d'auto-découverte (`scrapers/__init__.py`). Glisse-dépose un scraper, l'app gère le reste.
- [ ] **C9. Tableau de bord Statistiques ludique (B12) :** Ajout de métriques amusantes sur la page `/stats` (estimation du temps de recherche épargné, équivalent en euros économisé sur DeepL, graphiques de répartition par scrapers).
- [ ] **C10. Gestion de la Résilience d'API :** Système de retry automatique avec attente exponentielle pour contourner le rate limiting lors des très gros batchs.
...
- [x] **C11. Traducteur Multi-API Résilient :** Couche d'abstraction (`translator.py`) combinant Microsoft Azure Translator (Moteur principal F0) et DeepL (Secours automatique en cas de quota dépassé).
- [x] **C12. Routage Dynamique & Pattern Factory :** Extraction automatique du type de bibliothèque Kavita (`Manga`, `Comic`, `Book`) couplée à un aiguillage dynamique des requêtes vers les scrapers spécifiques.
- [x] **C13. Scraper ComicVine Hybride :** Recherche adaptative par album (Issue) et résolution de la série parente (Volume) avec extraction de couverture spécifique d'album, fusion des résumés et filtrage des résidus HTML (anti-bruit).
- [x] **C14. Nettoyage Contextuel de Titre :** Logique de nettoyage adaptative dans `scrapers/__init__.py` selon le format du média (retrait propre des zéros de tri dans les comics).
- [x] **C15. Purge de Fournisseur (Nautiljon) :** Retrait définitif de Nautiljon du routage par défaut. Leurs administrateurs ont apparemment trop peur que leurs processeurs chauffent et abusent des bans IP Cloudflare. Place aux vraies APIs ouvertes et robustes.