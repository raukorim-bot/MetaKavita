...
* **Library denylist scope + wipe heal** — `DISABLED_LIBRARIES` applies to **auto-sync polling only** (not dashboard, manual batch, or webhook). First-save wipe no longer disables every library; residual total denylists are healed on the next successful index load. After each `:dev` push: `docker pull` + `compose up -d --force-recreate --pull always`.
