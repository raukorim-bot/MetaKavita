...
* **C63. Persistent batch queue** — Batch jobs survive container restart (SQLite `batch_queue` + boot hydrate). UI: Add to queue, File modal (remove / clear), Pause (detach RAM, keep SQLite), Resume/Run queue. Stop cancels the durable queue. Toast on add (dupes skipped). Tests: `tests/test_batch_queue_c63.py`.
...
* **C63. File batch persistante** — Les jobs batch survivent au redémarrage (SQLite `batch_queue` + hydrate au boot). UI : Ajouter à la file, modale File (retirer / vider), Pause (détache la RAM, garde SQLite), Reprise/Lancer. Stop annule la file durable. Toast à l’ajout (doublons ignorés). Tests : `tests/test_batch_queue_c63.py`.
