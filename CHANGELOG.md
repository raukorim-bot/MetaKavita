...
> **Hotfix (same 1.6.1 line):** BF52 + library denylist scope (sync-only) + wipe heal. No version bump.
...
* **Hotfix BF52** — Fresh installs can save Kavita URL/API key from the Config modal again; Docker Compose paste-play (`host.docker.internal`) documented; library denylist is sync-only (not dashboard) with wipe heal.
...
* **Library denylist scope + wipe heal** — `DISABLED_LIBRARIES` now applies only to **batch / auto-sync / webhook** (not the dashboard toolbar/list). First-save wipe (`SYNC_LIBRARIES_PRESENT` with no checkboxes) no longer disables every library; residual total denylists are healed on the next successful index load. After each `:dev` push: `docker pull` + `compose up -d --force-recreate --pull always`.
...
* **Kavita library sync filter** — Config → Planning: checkboxes to enable/disable Kavita libraries (`DISABLED_LIBRARIES` denylist). Empty = all enabled; new libraries stay on by default. Applies to **batch, auto-sync, and webhook** (dashboard list/toolbar always shows all libraries).
...
* **Hotfix BF52** — Setup neuf : sauvegarde URL/clé Kavita depuis la modal Config de nouveau fiable ; Compose paste-play (`host.docker.internal`) documenté ; dénylist biblios = sync only (pas le dashboard) + heal wipe.
...
* **Périmètre dénylist + heal wipe** — `DISABLED_LIBRARIES` ne s’applique plus qu’au **batch / auto-sync / webhook** (pas à la toolbar/liste du dashboard). Le wipe du 1er save (`SYNC_LIBRARIES_PRESENT` sans cases) ne désactive plus toutes les biblios ; une dénylist totale résiduelle est soignée au prochain chargement index OK. Après chaque push `:dev` : `docker pull` + `compose up -d --force-recreate --pull always`.
...
* **Filtre de bibliothèques Kavita** — Config → Planification : cases à cocher pour activer/désactiver les bibliothèques (`DISABLED_LIBRARIES`, dénylist). Vide = tout actif ; les nouvelles biblios restent actives. S’applique au **batch, auto-sync et webhook** (le tableau de bord affiche toujours toutes les biblios).
