...
- [x] **C29. Manual Review Mode (v1.6.1, local WIP):** Scrape → pending queue → pick modal (score gradient, keys 1–3, weak red) + optional edit, telemetry, session recap — not Event-pause worker.
...
- [x] **C29. Mode Review Manuelle (v1.6.1, WIP local) :** scrape → file pending → modale pick (gradient de score, touches 1–3, bande faible rouge) + édition optionnelle, télémétrie, récap de session — pas le worker Event-pause.
