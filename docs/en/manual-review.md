...
If **Edit before confirm** is on (`MANUAL_REVIEW_EDIT`), **Adjust before send** is the last recap: cover URL, title, localized title, year, status, age rating, format, publisher, summary, genres, tags, staff — with the Master / Fusion bar. Each writable field has a tick: on means it is written, off leaves Kavita as-is. Title and format are display-only. A series **Targeted fields** override that dropped a field locks that tick. Footer: **Back**, **Skip**, **Confirm** (`Enter` / `Esc` / `Backspace`).
