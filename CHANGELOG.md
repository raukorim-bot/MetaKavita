...
> **Hotfix (same 1.6.1 line):** BF52 + library denylist = auto-sync only + wipe heal. No version bump.
...
* **Hotfix BF52** — Fresh installs can save Kavita URL/API key from the Config modal again; Docker Compose paste-play (`host.docker.internal`) documented; library denylist is **auto-sync polling only** with wipe heal.
...
* **Library denylist scope + wipe heal** — `DISABLED_LIBRARIES` applies to **auto-sync polling only** (not dashboard, manual batch, or webhook). The denylist no longer lives in `KavitaAPI.get_all_series()`, which always returns the full inventory; the auto-sync worker filters its own candidates (`select_auto_sync_candidates`). Side effect fixed: the polling loop used to purge the cache of every series in a disabled library as if it were orphaned. Series now always carry `libraryId`, so that filter can't silently miss. First-save wipe no longer disables every library; residual total denylists are healed on the next successful index load. After each `:dev` push: `docker pull` + `compose up -d --force-recreate --pull always`.
...
* **Hotfix BF52** — Setup neuf : sauvegarde URL/clé Kavita depuis la modal Config de nouveau fiable ; Compose paste-play (`host.docker.internal`) documenté ; dénylist biblios = **auto-sync only** + heal wipe.
