...
- [x] **BF66. Tag/genre dedupe before MAX caps (v1.6.2, issues #24/#25):** Case-insensitive order-preserving dedupe in `build_kavita_payload` before tag/genre slices.
- [x] **BF67. Soft atomicity for general fields (v1.6.2, issues #24/#25):** `update_series_general` only after successful metadata write.
- [x] **BF68. Score-tie prefers safer age (v1.6.2, issues #24/#25):** Auto demotes pornographic/erotica only on equal score (with log); MR unchanged; CBW+tie → awaiting_pick.
...
- [x] **BF66. Déduplication tags/genres avant plafonds MAX (v1.6.2, issues #24/#25) :** dédup insensible à la casse, ordre conservé, dans `build_kavita_payload` avant les tranches tags/genres.
- [x] **BF67. Atomicité soft des champs généraux (v1.6.2, issues #24/#25) :** `update_series_general` seulement après succès metadata.
- [x] **BF68. Égalité de score : préférence âge plus safe (v1.6.2, issues #24/#25) :** Auto rétrograde pornographic/erotica seulement à score égal (avec log) ; MR inchangé ; CBW+égalité → awaiting_pick.
