...
* SQLite `lifetime_stats` keys: `series_enriched`, `matches_won`, `series_missed` via `record_enrichment_telemetry` / `record_enrichment_miss`. C94 adds monotonic `covers_applied` (`mark_cover_manual`), `locks_sealed` (user 🔒 routes only — not auto-seal), and `runs_batch` / `runs_webhook` / `runs_auto` / `runs_row` (`record_run_origin` after a real Kavita write, same bar as `_REAL_SEND_MESSAGES`). C98 adds `auto_sync_waves` / `auto_sync_waves_stopped` / `auto_sync_waves_scan` / `auto_sync_waves_interval` / `auto_sync_series` / `auto_sync_ok` / `auto_sync_errors` / `auto_sync_review` / `auto_sync_relock` / `auto_sync_stopped` when a wave closes (`finish_open_auto_sync_run`, once). `get_lifetime_stats()` returns 0 for missing keys. No backfill.
...
* Playful `/stats` (`ENABLE_PLAYFUL_STATS`, default ON): `services/stats_service.py` + Chart.js + Manual Review achievements (`services/mr_achievements.py`). Standalone document: `static/css/stats.css` + `static/js/stats.js` after `style.css` — never `.dashboard-wrapper` / `.content` (BF176). Average manual score: `scored_average` (0–1). If `manual_score_n` started after `manual_score_sum`, a tiny n used to print 1200 % (BF177).

#### F. Batch progress bar (v1.6.1, isolated counters hotfix)
* Queue items are dicts built by `services/background_tasks.py::make_sync_item(series_id, series_name, force_update, fields_override=None, is_batch=False, origin=…)` — **not** tuples anymore. `origin` is `batch` / `webhook` / `auto` / `row` (default `batch` if `is_batch`, else `row`). `sync_queue` is still shared with webhook and auto-sync producers, so `is_batch` is what lets the worker tell a batch job apart from the others sharing the same queue.
...
| `services/enrichment_engine.py` | Manual branch in `enrich_series`, `preview_manual_review` / `apply_manual_review` / `research_manual_review` (apply shares `_processing_lock`; research piggybacks if the scrape already holds it); `seal_series_locks` for `NEEDS_RELOCK` |
...
* **Re-search a new title while the first MR / Super scrape still holds `_processing_series_ids` (BF189)** — `research_manual_review` is a candidate re-scrape, not a Kavita write. It bumps `_review_scrape_epoch` so in-flight streamed cards and `finalize_streaming_review` become no-ops, and it piggybacks the lock instead of returning « Déjà en cours de traitement ». `apply_manual_review` / `enrich_series` stay exclusive. The title field stays editable on the waiting mask until a re-search is already in flight.
...
* **Reliable Manual Review Flow & Option Isolation (Dashboard & Workshop)** — Resolves premature modal closure on candidate choice. The choice endpoint returns `mode="preview"` when edit before confirm is enabled (default), allowing users to step through pick → cover picker (if enabled) → preview & edit metadata before confirming. In the Workshop, `prefer_edit` is preserved and honored via `window.WORKSHOP_CONFIG` (inheriting `MANUAL_REVIEW_EDIT` and `MANUAL_REVIEW_COVER_PICK`), and confirmation stages changes directly onto the workshop series card (`save_workshop_series_override` -> `workshopApplyReview`) without writing to Kavita. On the dashboard, `syncOptionsFromSidebar` reads `data-mr-edit` from the DOM to avoid being overwritten by the `CONFIRM_BEFORE_WRITE` toggle when `MANUAL_REVIEW_MODE` is unticked, preventing the "Pause café" recap screen from appearing prematurely with 0 confirmations.
...
**C86 field picks** — `/choice` and `/confirm` accept `manual_completion`, `merge_fields`, `field_picks` (`{cover: ["AniList"], tags: ["MAL", "MU"]}`). When `field_picks` is present, `choice_and_merge` uses `apply_field_picks` (overwrite / list concat) instead of Source hole-fill. Age is bumped **per card blob** (`apply_explicit_label_age` on each `card["data"]`) **before** picks; the assembler does not bump after the mix (MAL Hentai genres must not rewrite a ComicVine age). Preview stores `_field_picks` / `_merge_fields` / `_manual_completion` for reopen. Bulk-accept still sends `include_providers=[]` with no picks. Tests: `tests/test_manual_completion_field_picks.py`.
...
**C88 Auto field mapping** — Auto/batch only. Shared assembler `services/field_assembly.py`; plan `services/field_mapping.py`; fetch `services/field_mapping_fetch.py`. `GET`/`POST /api/config/field-mapping` (JSON, not the global FormData). `load_config()` must overlay `FIELD_MAPPING_*` / `FIELD_PROVIDER_MAP_*` or a sidebar save wipes them. Dropdowns and fetches use Comic **or** Manga **or** Book — **never** `get_by_type("ComicFlexible")`. Manual / Super Review / Companion Super Review read **zero** `FIELD_MAPPING_*`; Companion **Auto** uses the map. `forced_provider != AUTO` skips mapping. The dashboard chip is C89 (any Options setting: forced provider / Magic ID, targeted fields that are not “all”, alternative title different from the Kavita name, publisher pref, localized-title langs). C91 reuses cascade blobs for mapping and fetches remaining override providers in parallel; Manual / Super Review use the same two-wave collect when Smart Scoring is off. Write policy unchanged: holes only unless Force. Tests: `tests/test_field_mapping.py`, `tests/test_field_mapping_config.py`, `tests/test_field_mapping_fetch.py`, `tests/test_field_mapping_enrich.py`.
...
#### I. Supporter nags (C40 partial, C109)
* `static/js/license_nag.js` — rare Buy Me a Coffee overlays after hot moments (batch end / rich MR recap / successful workshop writes). Caps: honeymoon 7d, activity threshold >= 10 items (series or volumes/reviews), max 1–2/day, honor snooze 30d. Failures must be no-ops (never block batch/MR/workshop). Class `.license` reserved as a future silence hook — no paywall / license keys.
...
* **Workshop support button & nagware (C109)** — `workshop-nav` includes a permanent amber button (`#workshopCoffeeBtn`, `.workshop-btn-coffee`) opening Buy Me a Coffee in a new tab with an immediate toast and auto-cooldown registration (`mk_nag_bmc_clicked_at`). In addition, `onWorkshopComplete()` triggers the supporter overlay with the dedicated `workshop_craft` variant after successful series or volume writes to Kavita.
...
Before this fix, `websocket.js` guessed `NOT_FOUND`/`PENDING_REVIEW`/skip-`COMPLETED` badges by pattern-matching **translated** keywords inside the raw `log_update` text ("réussi", "déjà à jour", "introuvable"...). That block is gone — `log_update` only appends a structured line to the sidebar journal (C90). Do not resurrect substring matching on log text for anything the badge needs to reflect; add a new `status` value and an `_emit_series_status()` call at the source instead. The `.is-processing` highlight follows `batch_progress.series_id` in `batch.js`, not the log. `uncheckSeriesForBatchResume()` (QoS batch resume) is likewise now driven by the `series_status` handler (`status === 'COMPLETED' || status === 'NEEDS_RELOCK'`), not by log text.
...
#### N. Readable live log (C90)

The sidebar `#log-console` is no longer a Courier terminal. `websocket.js` (`parseLiveLog` / `appendLiveLog`) splits `HH:MM:SS | …` and the C84 `« Title » (id)` (optional `[…]`) into `.log-time` / `.log-series` / `.log-msg` via `textContent` — never `innerHTML`. Consecutive lines of the same series omit the chip. Pause (or scrolling up) stops follow-scroll; Clear empties the pane; the DOM is capped at 300 lines. Severity rails use `ERROR|❌|💥|🚫` / `WARNING|⚠️|⚠` / `✅|✔️` only — not translated words. Do not parse log text to drive `.is-processing`. Tests: `tests/test_live_logs_ui.py`, `tests/test_batch_processing_highlight.py`.

#### O. Series override chips (C89)

A row chip is the C65 cover-lock pattern: visible on the dashboard, click opens Options (`openSeriesOverride` in `overrides.js`). Any Options setting gets a chip: **forced provider** (or Magic ID with provider still AUTO) skips C88 mapping; **targeted fields** writes only the remaining boxes, on as soon as the mask is not “all” (`targeted_fields_is_granular` / `isGranularTargetedFields`); **alternative title** when it casefold-differs from the Kavita name (the panel prefills the name, so a same-name save is not a chip); **publisher pref** other than Auto (VF/VA or VO); **localized-title langs** when tags are set. Saving all boxes checked stores `ALL`, not the twelve-key CSV, or the fields chip would lie. After save, `syncSeriesOverrideBadges` redraws the live row; the virtual list rebuilds from `overrideBadgesHtml`. Tests: `tests/test_override_panel_ui.py`, `tests/test_batch_targeted_fields.py`.

#### P. Parallel fetch within one series (C91)

Still one series at a time; `throttle_provider` still serializes two calls to the *same* scraper. `fetch_metadata` attaches `_cascade_blobs` (`{blob, query, is_id}`) on the Auto master. Mapping (`fetch_overrides`) reuses a blob only when that query/`is_id` matches what the override would send (a Magic ID must not reuse AniList's answer to MAL's id), then HTTP-fetches the rest in parallel via `ThreadPoolExecutor` + copied contextvars. Manual Review / Super Review (`return_candidates=True`) always use the two-wave collect (#1 then the rest in parallel), including when Smart Scoring is off. Auto with Smart Scoring off stays sequential so P2 still sees P1's ISBN. Pop `_cascade_blobs` before Kavita. Tests: `tests/test_field_mapping_fetch.py`, `tests/test_metadata_fetcher_smart_scoring.py`.
...
| `BABELIO` | Babelio | Book | French literature (sitemaps babmap_1–37 + `/livres/`); covers + summaries, no key. |
...
*   **`services/enrichment_engine.py`**: `enrich_series(series_id, series_name, force_update, targeted_fields_override=None)`, the extracted former `process_series_logic()`. Pure orchestration — scraping, field mapping, Kavita calls, lifetime telemetry broadcast — with zero dependency on Flask or `app.py`. Also hosts Manual Review apply/preview/research paths (C29) and `seal_series_locks` (`NEEDS_RELOCK`). Comic Flexible cascade lives here. Auto C88 mapping is gated here (`mapping_applies_here` → `run_mapping_wave`).
*   **`services/field_assembly.py`**: C86/C88 assembler — `assemble_field_picks`, `ProviderFieldSource`, `absorb_identity`. No HTTP, no `load_config`.
*   **`services/field_mapping.py`**: C88 config → `MappingPlan`. `dropdown_providers` never calls `get_by_type("ComicFlexible")`.
*   **`services/field_mapping_fetch.py`**: C88 Auto fetch (default + sequential overrides + assemble).
...
*   **`services/background_tasks.py`**: the daemon workers (`sync_queue` consumer + periodic auto-sync poller) and `start_background_workers()`, called once from `app.py` at import time (unchanged single-worker-process behavior, required for Gunicorn `-w 1`). Queue items are **dicts** built by `make_sync_item(series_id, series_name, force_update, fields_override=None, is_batch=False, origin=…)` — no longer 3-/4-tuples. `is_batch` drives the dedicated `_batch_total`/`_batch_done`/`_batch_real_sends` progress counters (§4.F); `origin` (`batch`/`webhook`/`auto`/`row`) is counted on a real Kavita write only (C94). auto-sync skips `PENDING_REVIEW` and filters candidate libraries through `select_auto_sync_candidates()` — the **only** place `DISABLED_LIBRARIES` applies (§4.J).
*   **`services/stats_service.py`**: playful `/stats` metrics + Chart.js payload from lifetime counters + cache snapshot + Manual Review achievements (`mr_achievements.py`). Gated by `ENABLE_PLAYFUL_STATS`. `charts["status"]` is six values (`COMPLETED`, `NEEDS_RELOCK`, `PENDING`, `PENDING_REVIEW`, `NOT_FOUND`, `IGNORED`) — `templates/stats.html` `labelsStatus` and the library mosaic must stay in that order (BF174). Chapters must `overflow: visible` and `min-height: 100dvh` (BF175: `overflow: hidden` clipped mosaics). The page is a standalone document (`static/css/stats.css` + `static/js/stats.js` after `style.css`); do not wrap it in `.dashboard-wrapper` / `.content` (BF176: those classes lock 100vh and clip the story). C92 adds Inventory / volume / mapping / Review-gesture snapshots; `cover_manual` counts protected covers. C93 adds homemade titles, language palette, pardoned duplicates, forced expected counts, TGV / sleep conversions, a shelf in cm, and the volume-write champion (`summarize_volume_writes`). C94 surfaces `covers_applied`, `locks_sealed`, and `runs_*` origins from lifetime (zeros if missing). C98 adds `playful["auto_sync"]` from closed-wave keys (zeros if missing; time saved for that chapter only). BF178: volume duration is 1 min of paste per written unit, not 25 min of reading. BF179: lifetime hit rate / avg matches stay on lifetime (0 if empty); `charts["hit_miss"]` is always cache completed vs not-found; manga pages = written volumes × 180; protected covers = `cover_manual` only; time saved adds volume typing minutes.
*   **`services/scraper_diagnostics.py`**: `/diagnostics` preflight + probes. C95: `probe_all` / page load test all targets in parallel. BF193: worker concurrency is bounded to `DEFAULT_PROBE_WORKERS = 4` (`ThreadPoolExecutor`) with real-time NDJSON streaming, avoiding CPU/GIL starvation on the single-process Eventlet worker. `_probe_reachability` and `probe_internet` use `stream=True` to read headers without downloading full response bodies (such as Babelio's 5 MB XML map). Cooperative `yield_to_worker()` checkpoints are placed between probe phases and results. “Test cascade” remains `scope=active`. Frontend `setPill` eliminates forced `offsetHeight` reflows.
...
*   **`static/js/*.js`**: the former monolithic `script.js` is now plain `<script>` files loaded in dependency order (`utils.js` → `websocket.js` → `overrides.js` → `covers.js` → `config.js` → `field_mapping.js` → `batch.js` → `manual_review.js` → `license_nag.js` → `main.js`). No bundler and no `type="module"` on purpose: templates rely on inline `onclick="..."` handlers, which require every function to stay in the global scope. `websocket.js` owns the C90 live-log DOM (`parseLiveLog` / `appendLiveLog`); it must not parse log text for badges or `.is-processing`.
*   **`static/css/style.css` (CSS manifest)**: entry-point for all styles — 21 `@import` statements in cascade order. The former 6 864-line monolith was split into **21 focused modules** under `static/css/base/`, `static/css/layout/`, `static/css/components/` and `static/css/modals/`. 204 lines of confirmed dead CSS (26 orphaned classes, 9 groups) were also removed. A byte-for-byte backup is kept at `static/css/style.backup.css`. Do not edit `style.css` directly — edit the relevant module and rely on the `@import` chain. Module map:
    * `base/_variables.css` — CSS custom properties (`:root` tokens, dark-theme overrides)
    * `base/_reset.css` — universal box-sizing, `body`, background gradients
    * `layout/_dashboard.css` — `.dashboard-wrapper`, `.topbar`, sidebar shell, Companion card
    * `layout/_sidebar.css` — scraping options, toggles, accordions, stats widget
    * `layout/_toolbar.css` — toolbar, filters, bulk-action bar, batch progress
    * `layout/_responsive.css` — all `@media` breakpoints
    * `components/_forms.css` — inputs, search bar, textarea, segmented control
    * `components/_buttons.css` — button variants, dropdown menus
    * `components/_series-card.css` — series cards, grid/list layout, pulse animation
    * `components/_inventory-badge.css` — inventory badges C66, volume status chips
    * `components/_dashboard-widgets.css` — stat tiles, advanced options pane, empty state
    * `components/_live-log.css` — live journal C90
    * `components/_alerts.css` — toasts, alerts, custom scrollbar
    * `components/_supporter.css` — Buy-Me-A-Coffee overlays and ribbons
    * `modals/_modal-base.css` — `.modal` base structure, backdrop, animations, audit report
    * `modals/_modal-inventory.css` — inventory modal, volume enrichment tables
    * `modals/_modal-manual-review.css` — manual review modal (candidates, diff, smart completion)
    * `modals/_modal-secondary.css` — options, confirmation, field-selector modals
...
*   **`static/css/volumes.css` (Workshop CSS manifest)**: entry-point for all workshop styles — 14 `@import` statements in cascade order. The former 1 578-line monolith was split into **14 focused modules** under `static/css/workshop/`. A byte-for-byte backup is kept at `static/css/volumes.backup.css`. Do not edit `volumes.css` directly — edit the relevant module and rely on the `@import` chain. Module map:
    ...
    * `workshop/_rail.css` — left series rail, search toolbar, status chips, virtual scroll layout
    * `workshop/_main.css` — central content container, loading overlay, background pass banner
    * `workshop/_series-card.css` — series card (slate glass, dirty halo, primary metadata, action buttons)
    * `workshop/_covers.css` — book cover component (2:3 book ratio, spine sheen, live search picker modal)
    * `workshop/_forms.css` — slate inputs, cascade `.workshop-cascade-btn`, gentle amber glow, More accordion
    * `workshop/_action-bar.css` — sticky volume floating action bar, selection counts, bulk send buttons
    * `workshop/_volume-cards.css` — volume cards, status chips (DONE/STAGED/PENDING), provider chips, Magic Input
    * `workshop/_candidates.css` — volume review candidates and manual field completion
    * `workshop/_logs.css` — live journal terminal and workshop history list
    * `workshop/_modals.css` — volume review modal overrides and `#manualReviewModal` isolation
    * `workshop/_responsive.css` — responsive media breakpoints (1100px, 860px, 520px) & `prefers-reduced-motion`
...
One network call for the whole series wherever the provider allows it — ComicVine's `/api/issues/?filter=volume:X&limit=100` returns a hundred issues, summaries and covers included, so a 150-issue run costs two calls. `fetch_volume` (one call per volume) exists for a Magic Input and for providers that cannot list. `wanted_numbers` is the set of Kavita numbers already normalised (`number_key`); HTML providers only GET those albums, then cap at `VOLUME_INDEX_MAX`. `should_cancel` is tested **inside** the album/page loop; a Magasin still on the four-argument signature is called via `inspect.signature` (`_call_volume_index`). A cancelled crawl returns the partial index already built, not `None`.

Payload keys: `title`, `summary`, `release_date`, `isbn`, `cover_url`, `provider_ref`. Declare `scopes = {"series", "volume"}` so `ScraperRegistry.get_by_scope("volume")` picks the scraper up. HTML providers must cap their walk (`VOLUME_INDEX_MAX = 50` on Bédéthèque and Planète BD at 2.5 s, **40 on Manga-News at 6 s**): an uncapped index is a quarter of an hour of silent scraping, and on Manga-News it is also how you get Cloudflare to block the address. `fetch_index` does not stop on a cover-only answer: MangaDex listing every volume by its cover must not prevent Manga-News from being asked for titles and summaries. The converse also holds: Stop after a covering text index keeps that index and does not then call `_append_first_cover_only`.
...
| `services/volume_enrichment/job.py` | Library-wide pass in a dedicated thread. Overlays workshop Magic overrides after `resolve_index` |
| `services/workshop.py` | Workshop payload, Kavita cover URLs, send grains, Magic, volume Review, reset |
| `services/kavita_cover_cache.py` | Disk cache for Kavita covers already fetched (`DATA_DIR/kavita_covers`) |
| `services/workshop_form.py` | Full Kavita series metadata form + volume extras; force overwrite on send; one metadata POST |
| `routes/volume_enrichment.py` | Preview / apply / library pass / status / cancel, all behind a 403 guard |
| `routes/workshop.py` | Workshop API + `GET /api/kavita-cover/...` (session Meta, never the API key) |
...
Workshop send is always force: a proposed value overwrites what Kavita already holds, including locked fields. An empty proposed value is never written (including `localizedName`). The series card is the full Kavita metadata sheet (year, status, age, language, publisher, links, genres, tags, staff) and leaves in **one** `POST /api/Series/metadata`. Empty fields stay visible and amber (`.workshop-field--empty`). Extra staff sit behind More (`details.workshop-more`), open state in `localStorage` (`workshop_show_more`). Short fields (year, status, age, language, staff) share a 6-column grid (three to a row). Magic Input is a labelled row on every volume card. Covers are a 2:3 book with a Pick overlay; `startCoverSearch` fills a pending `data-cover-url` and Send writes it (`upload_series_cover` / chapter cover), not `POST /update-cover`. The series title is a link to `{KAVITA_UI_URL}/library/{libraryId}/series/{id}`. The volume bar is `position: sticky` inside `#workshopMain`. The live log (`#log-console`) is a third column; `websocket.js` already binds it. `/` focuses the rail search; prev/next disable at the ends of the filtered list. Series **Manual Review** / **Super Review** set `window.WORKSHOP_FORCE_REVIEW` (this page has no sidebar checkboxes, so `isReviewQueueModeOn()` would otherwise stay false), open the waiting modal via `mrPrepareForBatch()`, then POST `/force-sync`. The queue is scoped to that series (same filter as Companion), so an already-pending file does not open in place of the series on screen. `sync_settled` calls `mrOnSyncSettled` when `settleSingleSync` is absent (`batch.js` is not loaded here). Confirm on this page posts `workshop: true`; `/confirm` fills the sheet and a pending cover and does not write Kavita (`apply_manual_review(..., workshop=True)`). Volume Review fills `#mrAboveList` with `data-kind=volume` and forces the pick panel visible; confirm saves the unit override and fills the card (`confirm_volume_review`), still without a Kavita write. Reusing `#manualReviewModal` for volumes is strictly isolated from series reviews (`isModalOpen()` ignores volume reviews, BF194); dedicated volume keyboard navigation (Escape, Enter, ArrowUp/Down, 1–9, double-click) is bound in `volumes.js`. Candidates fetched via ISBN and title queries carry `provider` and `provider_ref`. Auto-saving series drafts (`draft-series`) merges existing `_external_ids` and cover URLs. Staged volumes (`_staged: True`) retain their `data-dirty="1"` state and bar counters across reloads (F5) and rail navigation; series overrides stay dirty. Closing the modal clears the flag. The gesture journal names the volume, not `#chapter_id`. The automatic pass still only writes `INDEX_FIELDS`. `SKIPPED` / `NOTHING_FOUND` is HTTP 200 with `noop`, not 500. `_pass_blocks` is true only when the automatic pass is on *this* series. A per-volume Reset also clears that chapter's `volume_unit_cache` row so the pass does not treat it as settled. `send_selection` holds `claim_series_write` for the whole selection and records `sent` and `total` in history. Single sends clean card attributes via `markDoneClean([data])` and notify `SupporterNag`. Defensive guards prevent orphan calls when `seriesId` is null. A reload after send keeps dirty cards and tick state. `workshop_payload` carries `skipped_reason` (`oneshot` / `specials`) so the workshop banner is not an empty list; Magic/Review overrides fill `inscribed` so Send writes what the form shows. The rail shares the dashboard localStorage filter keys; a click loads `GET /api/series/<id>/workshop` into the main pane (`pushState`, no reload). `GET /api/kavita-cover/...` serves `DATA_DIR/kavita_covers` and sends Cache-Control. Tests: `tests/test_workshop.py`, `tests/test_volume_unit_cache.py`, `tests/test_workshop_audit_fixes.py`.
...
`matching.unmatchable_reason(units, series_name)` runs in the provider paths — `_enrich_one_series_locked` for the pass, `build_series_plan` for a single-series write — right after the volumes are read and **before** anything hits the network, and in `workshop_payload` for the banner (no network). It returns `oneshot`, `specials` or `""`.
...
**Light mode: hiding a category must switch its feature off (C80).** `UI_SHOW_MANUAL_REVIEW`, `UI_SHOW_INVENTORY`, `UI_SHOW_VOLUMES` and `UI_SHOW_FIELD_MAPPING` (C88, **off by default**) remove a sidebar category, listed on `<body data-ui-hidden="…">` as space-separated words and read by `[data-ui-hidden~="manual"] .so-cat[data-so-cat="manual"]`. One attribute rather than three because `data-ui-inventory` next to `data-inventory` would not say which one hides and which one switches off. The pairing itself is the load-bearing part: a category that has left the screen commands nothing, and two of these three write to Kavita — a hidden manual review left on fills a queue nothing empties, a hidden volume pass left on keeps a write button in the toolbar whose forced provider can no longer be changed. Mapping is the same pairing (`FIELD_MAPPING_ENABLED`). `config_manager.apply_light_mode()` holds the rule and `LIGHT_MODE_FEATURES` holds the mapping; it runs on **read** (so an environment variable or a hand-edited `config.json` cannot produce a running feature with no reachable switch) and again in `routes/config.py` on save, **before** the manual-review purge, which compares `was_manual` against the final value — otherwise hiding the category would switch the mode off and strand its queue. Dependent manual-review flags are switched off too, because `routes/companion.py` reads `MANUAL_REVIEW_COVER_PICK` on its own; the volume dependents are left as they are, since they only apply inside a pass that no longer starts, and keeping them gives the user their choices back. `onUiSectionToggle()` mirrors the rule on screen: it unticks the sidebar master, sets `data-inventory` / `data-volumes` to `0`, and saves once. It never re-enables anything on the way back — a revealed category comes back off. Tests: `tests/test_ui_light_mode.py`.
...
**The Kavita scan hub is the exception that must not be a greenthread.** Flask-SocketIO already owns the *server* WebSocket. A SignalR *client* on a greenthread freezes Live Logs and HTTP (same family as `debug/repro_cover_eventlet.py`). `services/kavita_hub.py` therefore runs on `eventlet.patcher.original("threading")` with unpatched sockets: JWT, negotiate and the hub loop stay off the eventlet hub. Callbacks only push `"scan"` onto `auto_sync_wake_queue`; `get_all_series` and enqueue stay on the existing Auto-sync worker. Never log the plugin JWT. Tests: `tests/test_kavita_hub_parse.py`, `tests/test_kavita_hub_live.py`.

Each Auto-sync enqueue wave writes `auto_sync_runs` / `auto_sync_run_items` (C97). The worker records `origin=auto` jobs only; Stop marks waiting auto jobs `stopped`. Closing a wave also bumps `auto_sync_*` lifetime keys once (C98). The dashboard status filter `AUTO_SYNC` (C99) keeps only those `series_id`s. The topbar button is unread-or-running, not a dashboard-batch recap. Not series-scoped: orphan cleanup must not drop the report. Tests: `tests/test_auto_sync_report.py`.
...
#### H. Inventory duplicates are a script, volumetric comparison, and Kavita scan (C85, C110)

`DELETE /api/Series/{id}` only drops the database row. The files stay; the next library scan recreates the series. Meta does not mount Kavita media and must not move users' files without consent. To solve this gracefully:

* Meta generates safe deletion/move scripts in both POSIX Bash (`.sh`) and Windows PowerShell (`.ps1`) (`services/library_audit/dup_script.py`). Meta never executes the script itself.
* Duplicate groups display real volume and chapter counts for each copy, compute `recommended_keep_id` (the most complete copy), and display a "🌟 Recommended (most complete)" badge. The UI displays an explicit confirmation dialog if the user attempts to trash a copy that has more volumes/chapters than the kept copy.
* After running the script on their host/NAS, users can click **⚡ Trigger Kavita Scan** (`POST /api/libraries/<library_id>/kavita-scan`) directly from the Duplicates modal in MetaKavita to trigger Kavita's library scanner via `KavitaAPI.scan_library()` without opening the Kavita web interface.
* If moving/deleting files on disk leaves an empty series shell in Kavita (0 volumes), MetaKavita provides `POST /api/series/<id>/purge-empty` to safely delete the empty shell via `KavitaAPI.delete_series()`. This route strictly verifies `KavitaAPI.is_series_empty()` beforehand (refusing series with existing volumes or chapters), and triggers surgical single-series cache eviction (`purge_single_series_from_all_caches`) without touching other series.
* Single-series removals (purge-empty, SignalR `SeriesRemoved`) must ALWAYS call `purge_single_series_from_all_caches(series_id)` rather than `clean_orphaned_cache()`, which requires a complete set of active series IDs.
* Path normalization (`normalize_inventory_folder_trash`) supports both POSIX (`/...`) and Windows drive letter (`C:/...`, `D:\...`) paths for PowerShell `.ps1` duplicate-drop scripts.
* Resolved duplicates can be dismissed via **Mark as resolved** (`POST /api/libraries/<library_id>/duplicates/dismiss` with `reason="resolved"`), and dismissal updates are synchronized across both the specific library and the global `'all'` view.
* Cache orphan cleanup (`clean_orphaned_cache`) runs automatically at the end of library scans, pruning deleted series from `series_cache`, `volume_report_cache`, and adjusting/deleting reduced groups in `duplicate_group_cache`.
* The SignalR hub (`services/kavita_hub.py`) listens for `SeriesRemoved` events and surgically purges local caches for that series via `purge_single_series_from_all_caches`.
* The prefix is a POSIX or Windows path (`INVENTORY_FOLDER_PATH_PREFIX`, `resolve_script_folder_path`), not an HTTP URL.
* Tests: `tests/test_dup_folder_script.py`, `tests/test_library_audit.py`.
...
#### C. Manual Review in Workshop: Staging Contract & Step Flow
- When Manual Review or Super Review is invoked from the workshop (`html.workshop-page`, `window.WORKSHOP_FORCE_REVIEW = true`):
  - The review goes through the complete review lifecycle: candidate pick → cover selection (if enabled) → metadata preview/edit (`phase = "edit"`).
  - Settings are inherited from user preferences via `window.WORKSHOP_CONFIG` (honoring `MANUAL_REVIEW_EDIT` and `MANUAL_REVIEW_COVER_PICK`).
  - Upon user confirmation (`POST /api/manual-reviews/<id>/confirm` with `workshop: true`):
    - Edits and cover are staged onto the series card (`save_workshop_series_override` → `workshopApplyReview`).
    - Review status in the queue becomes `PENDING` (not `COMPLETED`) until the user explicitly clicks "Send" on the series card.
...
- *Defensive API tuple unpacking*: `send_series` inspects the return value of `update_series_general` and `upload_series_cover` safely as a generic sequence, avoiding `ValueError: too many values to unpack` on 3-tuples `(ok, detail, sealed)` or 2-tuples.
- *Select dropdown contrast in dark mode*: Native select popups on Chromium/Windows default to light listbox backgrounds without `color-scheme: dark;`. Declaring `color-scheme` and setting explicit opaque styles on `option`/`optgroup` (`#0f172a` in workshop, `#12151f` in dashboard) prevents unreadable light-on-white text in empty/amber or standard controls.
- *More fields completed counter*: The expandable `<summary>` for secondary fields displays a live completion ratio: `More fields (x filled / y)` (FR: `Plus de champs (x complétés / y)`), reacting in real-time to user typing, selects, and manual review staging.

#### F. Workshop Reliability & UX Audit Hardening (BF192)
- *Volume review modal*: `#manualReviewModal[data-kind="volume"]` displays a dedicated header context block (`#mrVolumeContext`) showing series name, volume number, and volume title while hiding series-only controls (`.mr-series-block`, `#mrQueueBadge`). Candidates render cover thumbnails, book-style metadata without orphaned separators, and pre-select the first candidate.
- *Dirty state lifecycle*: `syncDirtyState()` computes dirty state dynamically from current inputs and attributes. Successful writes (`workshopSendSeries`, `send_volume`, `markDoneClean`) cleanly remove `data-dirty`, `data-cover-url`, `data-cover-display`, and update `data-initial`. `snapshotDirty()` only captures series edits and volume covers if the corresponding card is genuinely dirty, eliminating stale-draft resurrection after Kavita writes.
- *Draft auto-save race isolation*: `scheduleSeriesDraft()` captures `targetSid = seriesId` in closure and verifies `seriesId === targetSid` upon timer firing. Navigating away (`goTo`, `loadSeries`) clears any pending timer, preventing edits on series A from writing to series B's draft in SQLite.
- *Sentinel date sanitization*: `inscribed_from_chapter` and `volumeFieldsOf` detect Kavita's default sentinel timestamp (`0001-01-01T00:00:00`) and sanitize it to empty string `""`, enabling correct amber empty-field highlighting.
- *Manual ISBN propagation in review*: `openVolumeReview` extracts the typed ISBN from the volume input and passes it to `POST /api/series/<id>/workshop/review`, which forwards it to `begin_volume_review` and `fetch_by_isbn`.
- *Live rail reactivity*: `updateRailStatus(sid, newStatus)` updates the rail in-memory and re-renders it on successful series send and via WebSocket `series_status` events.
...
- *Health Bar & Count Chips Preservation*: `applyDuplicateFlagsToUi` and `_updateHygieneCounts` merge partial counts and guard `_updateHygieneHealth` against missing `series`/`healthy` fields, ensuring the dashboard health bar never vanishes upon opening duplicates or dismissing groups.
- *Duplicate Dismissal Synchronization & Global Sharing*: `library_duplicates_dismiss` updates `hygiene_library_meta` in SQLite with the decremented duplicate count. `list_dismissed_group_keys` and `list_dup_dismissals` check dismissals across both specific library IDs and `'all'`, allowing duplicate dismissals to carry over seamlessly.
- *ComicVine & AniList Scraping Fixes*: `_comicvine_issues` and `series_identity.py` strip any leading `4050-` or `4050` entity prefix before formatting the URL. `resolve_catalog_expected` checks `not ids.get("anilist")` instead of `not ids`, allowing AniList title search when a series has a MAL or MangaBaka ID but lacks an AniList ID.
- *Modal Stacking & Ergonomics*: Pressing Escape evaluates modal visibility in priority order (`volumeReportModal` -> `duplicatesModal` -> `missingVolumesModal`), closing only the topmost open modal. Missing volumes table rows carry `data-series-name` to prevent title pollution (`#ID`), and un-analyzed library CSV/TXT requests return valid empty headers instead of JSON 404 downloads. Rich `/audit-badges` return `completion_state`, `forced`, and `unit` for instant badge color rendering.

#### I. Volume Enrichment & Workshop Lifecycle Hardening (C111, BF197)
- *Cover URL Cleaning Upon Send*: When `send_volume` successfully writes to Kavita (`status == "DONE"`), `cover_url` and `_staged` are purged from `clean_payload` before saving `volume_unit_overrides` in SQLite. This prevents cover resurrection on page reload (F5) and stops redundant cover re-uploads on subsequent volume saves.
- *Series Dirty Tracking Against Raw Baseline*: `seriesBaseline()` compares form inputs to `f.initial_value` (raw Kavita state) rather than `f.value` (which includes staged SQLite overrides), ensuring `seriesIsDirty()` correctly detects unsaved changes.
- *Persistent Volume Drafts (`POST /draft-volume`)*: User edits on any volume card automatically trigger a debounced (500 ms) auto-save to `/api/series/<id>/workshop/draft-volume`, writing `_staged: True` and `_source: manual` to SQLite. The automatic library pass (`_enrich_one_series`) checks `_staged` and skips staged units, preventing background runs from overwriting work in progress.
- *Exhaustive Multi-Provider Candidate Gathering (`all_scrapers=True`)*: `fetch_by_isbn` and `fetch_by_title_volume` support `all_scrapers: bool = False`. When called with `all_scrapers=True` by `begin_volume_review`, searches do not stop on the first hit, querying all active scrapers in parallel and indexing candidates as `{key}_{scraper.id}`.
- *Automatic Pass Resumption on Volume Reset*: Resetting a single volume via `clear_volume_unit_states(series_id, chapter_id)` deletes both the chapter row and the `SERIES_PASS_CHAPTER_ID` (`chapter_id = 0`) sentinel row, ensuring `list_enriched_series_ids()` does not treat the series as complete and allowing resumed library passes (`resume=True`) to re-process reset volumes.
- *Automatic Hygiene Cache Invalidation*: All workshop write operations (`send_volume`, `send_selection`, `send_series`) immediately call `purge_series_hygiene_cache(series_id, keep_overrides=True)`, keeping dashboard health chips and volume reports synchronized.
- *Date Normalization & Network Error Toasts*: ISO timestamps (`YYYY-MM-DDTHH:MM:SS`) are truncated to `YYYY-MM-DD` and sentinel `0001-01-01` is sanitized. Send button catch blocks display `toast(T().err_network)`.

#### J. Modern & High-Density Workshop CSS Redesign (C112)
- *Doubled Information Density & Prominent Covers*: Compact layout cuts vertical height across the workshop while giving covers a proud, modern presence. Series card height is halved (~700 px to ~380–420 px) with an elegant 124×186 px book cover and 26–28 px inputs. Volume cards are streamlined from ~450 px to ~190–210 px with prominent 104×156 px covers (aspect ratio 2:3) that match the vertical height of the metadata block, eliminating empty space. The checkbox is neatly placed adjacent to the cover, Champ Magique forms a sleek inline bar, field inputs are compacted to 24 px, and the "Choisir" button reveals its label on volume covers. Users can now comfortably view and compare 2–3 rich volumes simultaneously on standard displays without continuous scrolling.
- *Gentle Amber Glow for Empty Fields*: Replaced harsh, high-contrast bright yellow-orange borders (`rgba(251, 191, 36, 0.75)` + inset shadows) with a subtle, warm amber tint (`rgba(251, 191, 36, 0.38)` border, `rgba(251, 191, 36, 0.03)` background). Hover and focus provide soft amber rings without appearing as critical validation errors.
- *Modern Dark Glassmorphism & UI Accents*: Replaced muddy brown backgrounds with deep slate (`rgba(17, 24, 39, 0.75)` / `rgba(13, 20, 32, 0.85)`), layered with backdrop blur (12–16 px) and refined 1px borders (subtle teal for series, warm peach/amber for volumes). The sticky volume action bar (`#workshopBar`) floats cleanly with unified button groups and an orange gradient primary button. Left rail items are compacted from 56 px to 48 px (+15% vertical capacity). Full light-mode (`[data-theme="light"]`) fidelity is preserved.
- *JS Virtual Scroll & Keyboard Hardening*: Rail virtual scroll constant `ROW_H` is calibrated to 48 (matching the 48 px item height in `volumes.css`), eliminating scroll offset drift. `isTypingTarget` excludes non-input modal elements when `kind === 'volume'`, unblocking keyboard review navigation (Enter, ArrowUp/ArrowDown, 1–9). Staged volume covers picked from the modal automatically trigger `scheduleVolumeDraft` to persist across reloads. Strict `display: none !important;` guards prevent user-agent `[hidden]` overrides on broken or missing covers, and responsive layout maintains `18px 58px 1fr` down to 520px.

#### K. Series-to-Volumes Field Cascade & Modern Cards (C113)
- *Form Controls Inspired by Manual Review*: Reworks workshop inputs and select controls to match the sleek design of the Manual Review and "Adjust before send" screens. Replaces pitch-black void inputs with a modern dark slate background (`#141926`), smooth 7–8 px border-radius, generous 30 px height (`padding: 4px 10px`), and subtle 1px border (`rgba(148, 163, 184, 0.2)`). Labels switch from harsh uppercase to natural sentence case (`text-transform: none`, font size 11 px, font weight 600) with clean muted lock icons. Empty fields completely eliminate loud yellow outlines in favor of a soft, non-intrusive tint (`rgba(251, 191, 36, 0.26)` border, `rgba(251, 191, 36, 0.02)` background) that stays gentle on the eyes.
- *One-Click Field Duplication (`.workshop-cascade-btn`)*: All compatible series fields (`writers`, `pencillers`, `coverArtists`, `translators`, `genres`, `tags`, `ageRating`, `language`, `webLinks`, `summary`) feature an elegant micro-button in their header label. Clicking it reads the current series field value and cascades it instantly down to all loaded volume cards, syncing the input value, marking the affected cards as `data-dirty="1"`, scheduling background SQLite draft saves via `scheduleVolumeDraft`, and notifying the user with an instant confirmation toast (`workshop_cascade_success`).
- *Global Metadata Cascade Action (`#workshopCascadeAllBtn`)*: A dedicated action button equipped with a custom SVG cascade icon (`#mk-ico-cascade`) in the series card actions bar cascades all filled common metadata from the series across all volumes in a single click, with automatic dirty marking and draft persistence.
- *Seamless Series Cover Integration (Zero Void)*: Series card layout uses a 7-column continuous CSS grid (`136px repeat(6, minmax(0, 1fr))`) with `display: contents;` on `#workshopSeriesFields`. The 136×204 px book cover spans rows 1–3 adjacent to the title heading, localized name, and series summary. Lower fields (release year, status, age rating, publishers, genres, tags, writers, pencillers, More accordion, and action buttons) flow continuously across the full card width directly beneath the cover, completely eliminating any empty dead space below the cover.
- *Seamless Volume Cover Integration (Zero Void)*: Volume card layout uses an 8-column continuous CSS grid (`20px 104px repeat(6, minmax(0, 1fr))`) with `display: contents;` on `.workshop-volume-meta` and the primary `.workshop-volume-fields`. The 104×156 px volume cover and checkbox span rows 1–4 adjacent to the header, Champ Magique, title, ISBN, and release date. Lower elements (volume summary textarea, More accordion, and action buttons) flow continuously across the full card width (`1 / -1`) directly beneath the cover, completely eliminating any empty dead space below the volume cover.
- *Real-Time WebSocket Success Logging on Send*: Writing to Kavita via `send_volume` (single volume), `send_selection` (selected or all volumes), and `send_series` (series metadata card) emits formatted `logging.info(...)` logs recognized by `WebSocketLogHandler` and parsed by `websocket.js`. Each log includes the series label `[« Name » (id)]`, a green `✅` success indicator, the formatted unit label (e.g. `tome 2` / `volume 2`), and the list of written fields, bringing live visual feedback to the workshop logs console.
...
* Clés SQLite `lifetime_stats` : `series_enriched`, `matches_won`, `series_missed` via `record_enrichment_telemetry` / `record_enrichment_miss`. C94 ajoute `covers_applied` (`mark_cover_manual`, monotone), `locks_sealed` (routes 🔒 utilisateur seulement — pas le rescellement auto), et `runs_batch` / `runs_webhook` / `runs_auto` / `runs_row` (`record_run_origin` après une vraie écriture Kavita, même barre que `_REAL_SEND_MESSAGES`). C98 ajoute `auto_sync_waves` / `auto_sync_waves_stopped` / `auto_sync_waves_scan` / `auto_sync_waves_interval` / `auto_sync_series` / `auto_sync_ok` / `auto_sync_errors` / `auto_sync_review` / `auto_sync_relock` / `auto_sync_stopped` à la clôture d'une vague (`finish_open_auto_sync_run`, une fois). `get_lifetime_stats()` rend 0 si la clé manque. Pas de rattrapage.
...
* `/stats` ludique (`ENABLE_PLAYFUL_STATS`, défaut ON) : `services/stats_service.py` + Chart.js + hauts-faits Manual Review (`services/mr_achievements.py`). Document autonome : `static/css/stats.css` + `static/js/stats.js` après `style.css` — jamais `.dashboard-wrapper` / `.content` (BF176). Score moyen manuel : `scored_average` (0–1). Si `manual_score_n` a démarré après `manual_score_sum`, un n minuscule affichait 1200 % (BF177).

#### F. Barre de progression batch (v1.6.1, hotfix compteurs isolés)
* Les items de file sont désormais des dicts construits par `services/background_tasks.py::make_sync_item(series_id, series_name, force_update, fields_override=None, is_batch=False, origin=…)` — plus des tuples. `origin` vaut `batch` / `webhook` / `auto` / `row` (défaut `batch` si `is_batch`, sinon `row`). `sync_queue` reste partagée avec le webhook et l'auto-sync ; `is_batch` permet au worker de distinguer un job de batch des autres qui partagent la même file.
...
| `services/enrichment_engine.py` | Branche manuelle dans `enrich_series`, `preview_manual_review` / `apply_manual_review` / `research_manual_review` (apply partage `_processing_lock` ; research rejoint le verrou si le scrape le tient déjà) ; `seal_series_locks` pour `NEEDS_RELOCK` |
...
* **Re-rechercher un autre titre pendant que le premier scrape MR / Super tient `_processing_series_ids` (BF189)** — `research_manual_review` ne fait que re-collecter des candidats, pas une écriture Kavita. Il incrémente `_review_scrape_epoch` pour que les cartes streamées et `finalize_streaming_review` du scrape en vol deviennent no-op, et il rejoint le verrou au lieu de renvoyer « Déjà en cours de traitement ». `apply_manual_review` / `enrich_series` restent exclusifs. Le champ titre reste saisissable sur le masque d'attente tant qu'une re-recherche n'est pas déjà lancée.
...
* **Fiabilisation des étapes de Review Manuelle & Isolation des options (Tableau de bord & Atelier)** — Corrige la fermeture prématurée de la modale lors de la sélection d'un candidat. L'endpoint `/choice` renvoie `mode="preview"` lorsque l'édition avant confirmation est active (par défaut), garantissant le passage par les étapes normales : choix candidat → sélection jaquette (si active) → prévisualisation et ajustement des métadonnées avant confirmation. Dans l'Atelier, `prefer_edit` est préservé et respecté via `window.WORKSHOP_CONFIG` (héritant de `MANUAL_REVIEW_EDIT` et `MANUAL_REVIEW_COVER_PICK`), et la confirmation enregistre les choix en brouillon sur la fiche série (`save_workshop_series_override` -> `workshopApplyReview`) sans écrire dans Kavita. Sur le tableau de bord, `syncOptionsFromSidebar` lit l'attribut `data-mr-edit` dans le DOM pour ne pas être perturbé par la bascule de `CONFIRM_BEFORE_WRITE` lorsque `MANUAL_REVIEW_MODE` est décoché, empêchant l'apparition prématurée du récapitulatif de session « Pause café » avec 0 confirmation.
...
**C88 mapping Auto** — Auto / lots seulement. Assembleur partagé `services/field_assembly.py` ; plan `services/field_mapping.py` ; fetch `services/field_mapping_fetch.py`. `GET`/`POST /api/config/field-mapping` (JSON, pas le FormData global). `load_config()` doit overlay les `FIELD_MAPPING_*` / `FIELD_PROVIDER_MAP_*` sinon un save sidebar les efface. Dropdowns et fetches = Comic **ou** Manga **ou** Book — **jamais** `get_by_type("ComicFlexible")`. Review manuelle / Super / Companion Super ne lisent **aucun** `FIELD_MAPPING_*` ; le Companion **Auto** utilise la carte. `forced_provider != AUTO` saute le mapping. La cartouche dashboard est C89 (toute option Options : fournisseur forcé / ID magique, champs ciblés qui ne sont plus « tout », titre alternatif différent du nom Kavita, préférence d'éditeur, langues du titre localisé). C91 réutilise les blobs de cascade pour le mapping et fetch les overrides encore absents en parallèle ; Review / Super utilisent la collecte à deux vagues même si le Smart Scoring est off. Politique d'écriture inchangée : trous seulement sauf Force. Tests : `tests/test_field_mapping.py`, `tests/test_field_mapping_config.py`, `tests/test_field_mapping_fetch.py`, `tests/test_field_mapping_enrich.py`.
...
#### I. Pubs supporter (C40 partiel, C109)
* `static/js/license_nag.js` — overlays Buy Me a Coffee rares après moments chauds (fin de batch / récap MR riche / écritures réussies dans l'atelier). Caps : honeymoon 7 j, seuil d'activité >= 10 items (séries ou tomes/reviews), max 1–2/j, silence honor 30 j. Les échecs doivent être no-op (jamais bloquer batch/MR/atelier). Classe `.license` réservée pour un futur silence — pas de paywall / clé licence.
...
* **Bouton de soutien et nagware atelier (C109)** — `workshop-nav` intègre un bouton ambré permanent (`#workshopCoffeeBtn`, `.workshop-btn-coffee`) ouvrant Buy Me a Coffee dans un nouvel onglet avec toast immédiat et enregistrement du cooldown (`mk_nag_bmc_clicked_at`). De plus, `onWorkshopComplete()` déclenche l'overlay supporter avec la variante dédiée `workshop_craft` après des écritures réussies de série ou de tomes dans Kavita.
...
Avant ce correctif, `websocket.js` devinait les badges `NOT_FOUND`/`PENDING_REVIEW`/`COMPLETED`-skip en cherchant des mots-clés **traduits** dans le texte brut de `log_update` (« réussi », « déjà à jour », « introuvable »...). Ce bloc a disparu — `log_update` n'ajoute plus qu'une ligne structurée au journal de la barre latérale (C90). Ne réintroduis jamais de matching sur du texte de log pour ce que le badge doit refléter ; ajoute plutôt une nouvelle valeur de `status` et un appel `_emit_series_status()` à la source. Le liséré `.is-processing` suit `batch_progress.series_id` dans `batch.js`, pas le journal. `uncheckSeriesForBatchResume()` (reprise QoS batch) est désormais elle aussi pilotée par le handler `series_status` (`status === 'COMPLETED' || status === 'NEEDS_RELOCK'`), pas par du texte de log.
...
#### N. Journal live lisible (C90)

`#log-console` n'est plus une console Courier. `websocket.js` (`parseLiveLog` / `appendLiveLog`) découpe `HH:MM:SS | …` et le libellé C84 `« Titre » (id)` (`[…]` optionnel) en `.log-time` / `.log-series` / `.log-msg` via `textContent` — jamais `innerHTML`. Les lignes consécutives d'une même série omettent la pastille. Pause (ou un défilement vers le haut) arrête le suivi ; Vider vide le volet ; le DOM est plafonné à 300 lignes. Les lisérés de gravité ne regardent que `ERROR|❌|💥|🚫` / `WARNING|⚠️|⚠` / `✅|✔️` — pas les mots traduits. Ne parse pas le texte du journal pour poser `.is-processing`. Tests : `tests/test_live_logs_ui.py`, `tests/test_batch_processing_highlight.py`.

#### O. Cartouches d'override série (C89)

Une pastille de ligne, même geste que le 🔒 C65 : visible sur le dashboard, un clic ouvre les Options (`openSeriesOverride` dans `overrides.js`). Toute option Options a une cartouche : **fournisseur forcé** (ou ID magique avec le provider encore AUTO) saute le mapping C88 ; **champs ciblés** n'écrit plus que les cases restantes, dès que le masque n'est plus « tout » (`targeted_fields_is_granular` / `isGranularTargetedFields`) ; **titre alternatif** s'il diffère (casefold) du nom Kavita (le panneau préremplit le nom, donc pas de pastille si on enregistre le même) ; **préférence d'éditeur** autre qu'Auto (VF/VA ou VO) ; **langues du titre localisé** dès que des tags sont posés. Enregistrer toutes les cases cochées stocke `ALL`, pas le CSV à douze clés, sinon la pastille champs mentirait. Après save, `syncSeriesOverrideBadges` redessine la ligne ; la liste virtuelle reconstruit depuis `overrideBadgesHtml`. Tests : `tests/test_override_panel_ui.py`, `tests/test_batch_targeted_fields.py`.

#### P. Fetch parallèle dans une série (C91)

Toujours une série à la fois ; `throttle_provider` sérialise encore deux appels au *même* scraper. `fetch_metadata` pose `_cascade_blobs` (`{blob, query, is_id}`) sur le master Auto. Le mapping (`fetch_overrides`) ne réemploie un blob que si query/`is_id` sont ceux que l'override enverrait (un ID magique ne doit pas garder la réponse AniList à l'id MAL), puis refetch le reste en HTTP en parallèle (`ThreadPoolExecutor` + contextvars copiés). Review manuelle / Super (`return_candidates=True`) collectent toujours en deux vagues (#1 puis le reste en parallèle), y compris si le Smart Scoring est off. L'Auto scoring-off reste séquentiel pour que P2 voie encore l'ISBN de P1. Pop `_cascade_blobs` avant Kavita. Tests : `tests/test_field_mapping_fetch.py`, `tests/test_metadata_fetcher_smart_scoring.py`.
...
| `BABELIO` | Babelio | Book | Littérature FR (sitemaps babmap_1–37 + `/livres/`) ; covers + résumés, sans clé. |
...
*   **`services/enrichment_engine.py`** : `enrich_series(series_id, series_name, force_update, targeted_fields_override=None)`, extraction de l'ancien `process_series_logic()`. Logique d'orchestration pure (scraping, mapping des champs, appels Kavita, broadcast télémétrie lifetime) sans aucune dépendance vers Flask ni `app.py`. Héberge aussi les chemins apply/preview/research de la Review Manuelle (C29) et `seal_series_locks` (`NEEDS_RELOCK`). Cascade Comic Flexible ici. Le mapping Auto C88 est cadré ici (`mapping_applies_here` → `run_mapping_wave`).
*   **`services/field_assembly.py`** : assembleur C86/C88 — `assemble_field_picks`, `ProviderFieldSource`, `absorb_identity`. Zéro HTTP, zéro `load_config`.
*   **`services/field_mapping.py`** : C88 config → `MappingPlan`. `dropdown_providers` n'appelle jamais `get_by_type("ComicFlexible")`.
*   **`services/field_mapping_fetch.py`** : fetch Auto C88 (défaut + overrides séquentiels + assemblage).
...
*   **`services/background_tasks.py`** : les workers démons (consommateur de `sync_queue` + polling d'auto-sync périodique) et `start_background_workers()`, appelé une seule fois par `app.py` au chargement du module (comportement inchangé, requis pour un déploiement Gunicorn à worker unique `-w 1`). Items de file : des **dicts** construits par `make_sync_item(series_id, series_name, force_update, fields_override=None, is_batch=False, origin=…)` — plus des tuples 3-/4-. `is_batch` pilote les compteurs de progression dédiés `_batch_total`/`_batch_done`/`_batch_real_sends` (§4.F) ; `origin` (`batch`/`webhook`/`auto`/`row`) n'est compté que sur une vraie écriture Kavita (C94). L'auto-sync ignore `PENDING_REVIEW` et filtre ses candidats par bibliothèque via `select_auto_sync_candidates()` — le **seul** endroit où `DISABLED_LIBRARIES` s'applique (§4.J).
*   **`services/stats_service.py`** : métriques `/stats` ludiques + payload Chart.js à partir des compteurs lifetime + snapshot cache + hauts-faits Manual Review (`mr_achievements.py`). Piloté par `ENABLE_PLAYFUL_STATS`. `charts["status"]` a six valeurs (`COMPLETED`, `NEEDS_RELOCK`, `PENDING`, `PENDING_REVIEW`, `NOT_FOUND`, `IGNORED`) — `labelsStatus` et la mosaïque bibliothèque de `templates/stats.html` doivent rester dans cet ordre (BF174). Les chapitres doivent rester en `overflow: visible` et `min-height: 100dvh` (BF175 : `overflow: hidden` coupait les mosaïques). La page est un document autonome (`static/css/stats.css` + `static/js/stats.js` après `style.css`) ; ne pas l'envelopper dans `.dashboard-wrapper` / `.content` (BF176 : ces classes figent 100vh et coupent le récit). C92 ajoute les snapshots Inventaire / tomes / mapping / gestes Review ; `cover_manual` compte les couvertures protégées. C93 ajoute titres maison, palette de langues, doublons pardonnés, attendus forcés, conversions TGV / sommeil, une étagère en cm, et le champion des écritures de tomes (`summarize_volume_writes`). C94 expose `covers_applied`, `locks_sealed` et les origines `runs_*` du lifetime (0 si la clé manque). C98 ajoute `playful["auto_sync"]` depuis les clés de vague close (0 si la clé manque ; temps gagné de ce chapitre seulement). BF178 : la durée des tomes est 1 min de collage par unité écrite, pas 25 min de lecture. BF179 : le taux de hit / matchs par série lifetime restent sur le lifetime (0 s'il est vide) ; `charts["hit_miss"]` est toujours completed vs not-found du cache ; pages manga = tomes écrits × 180 ; couvertures protégées = `cover_manual` seulement ; le temps gagné ajoute les minutes de saisie des tomes.
*   **`services/scraper_diagnostics.py`** : préflight + probes de `/diagnostics`. C95 : `probe_all` / le chargement de la page sondent toutes les cibles en parallèle. BF193 : la concurrency est bornée à `DEFAULT_PROBE_WORKERS = 4` (`ThreadPoolExecutor`) avec flux NDJSON en temps réel, évitant la saturation CPU/GIL sur le worker unique Eventlet. `_probe_reachability` et `probe_internet` utilisent `stream=True` pour lire les en-têtes sans télécharger les corps de réponse complets (comme le sitemap XML de 5 Mo de Babelio). Des points de coopération `yield_to_worker()` sont posés entre chaque phase et à chaque résultat. « Tester la cascade » reste `scope=active`. Côté frontend, `setPill` supprime les reflows forcés `offsetHeight`.
...
*   **`static/js/*.js`** : l'ancien `script.js` monolithique est désormais découpé en fichiers `<script>` classiques chargés dans l'ordre de dépendance (`utils.js` → `websocket.js` → `overrides.js` → `covers.js` → `config.js` → `field_mapping.js` → `batch.js` → `manual_review.js` → `license_nag.js` → `main.js`). Volontairement sans bundler ni `type="module"` : les templates s'appuient sur des gestionnaires `onclick="..."` inline, qui exigent que chaque fonction reste en portée globale. `websocket.js` possède le DOM du journal C90 (`parseLiveLog` / `appendLiveLog`) ; il ne doit pas parser le texte du journal pour les badges ni pour `.is-processing`.
*   **`static/css/style.css` (manifest CSS)** : point d'entrée de tous les styles — 21 déclarations `@import` dans l'ordre de cascade. L'ancien monolithe de 6 864 lignes a été découpé en **21 modules ciblés** sous `static/css/base/`, `static/css/layout/`, `static/css/components/` et `static/css/modals/`. 204 lignes de code mort confirmé (26 classes orphelines, 9 groupes) ont également été supprimées. Une copie bit-à-bit de sauvegarde est conservée dans `static/css/style.backup.css`. Ne jamais modifier `style.css` directement — modifier le module concerné et s'appuyer sur la chaîne `@import`. Carte des modules :
    * `base/_variables.css` — propriétés CSS custom (tokens `:root`, surcharges thème sombre)
    * `base/_reset.css` — box-sizing universel, `body`, dégradés de fond
    * `layout/_dashboard.css` — `.dashboard-wrapper`, `.topbar`, coquille sidebar, encart Companion
    * `layout/_sidebar.css` — options de scraping, interrupteurs, accordéons, widget stats
    * `layout/_toolbar.css` — barre d'outils, filtres, barre d'actions de masse, progression batch
    * `layout/_responsive.css` — tous les `@media` breakpoints
    * `components/_forms.css` — inputs, barre de recherche, textarea, segmented control
    * `components/_buttons.css` — variantes de boutons, menus déroulants
    * `components/_series-card.css` — cartes séries, grille/liste, animation de pulsation
    * `components/_inventory-badge.css` — badges inventaire C66, puces de statut tomes
    * `components/_dashboard-widgets.css` — tuiles statistiques, panneau options avancées, empty state
    * `components/_live-log.css` — journal live C90
    * `components/_alerts.css` — toasts, alertes, scrollbar personnalisée
    * `components/_supporter.css` — overlays et rubans Buy-Me-A-Coffee
    * `modals/_modal-base.css` — structure `.modal` de base, backdrop, animations, rapport audit
    * `modals/_modal-inventory.css` — modale inventaire, tableaux d'enrichissement tomes
    * `modals/_modal-manual-review.css` — modale revue manuelle (candidats, diff, complétion)
    * `modals/_modal-secondary.css` — modales options, confirmations, sélecteurs de champs
...
*   **`static/css/volumes.css` (manifest CSS de l'Atelier)** : point d'entrée de tous les styles de l'Atelier — 14 déclarations `@import` dans l'ordre de cascade. L'ancien monolithe de 1 578 lignes a été découpé en **14 modules ciblés** sous `static/css/workshop/`. Une copie bit-à-bit de sauvegarde est conservée dans `static/css/volumes.backup.css`. Ne jamais modifier `volumes.css` directement — modifier le module concerné et s'appuyer sur la chaîne `@import`. Carte des modules :
    ...
    * `workshop/_rail.css` — rail gauche des séries, recherche, cartouches d'états, scroll virtuel
    * `workshop/_main.css` — conteneur central, overlay de chargement, bannière de passe d'arrière-plan
    * `workshop/_series-card.css` — fiche série (fond ardoise, halo dirty, métadonnées primaires, boutons d'action)
    * `workshop/_covers.css` — composant couverture (ratio livre 2:3, relief reliure, modale de recherche live)
    * `workshop/_forms.css` — inputs ardoise, cascade `.workshop-cascade-btn`, liseré ambré doux, accordéon Plus
    * `workshop/_action-bar.css` — barre d'actions flottante sticky des tomes, compteurs, boutons d'envoi groupés
    * `workshop/_volume-cards.css` — cartes de tomes, puces de statut (DONE/STAGED/PENDING), puces fournisseurs, Champ Magique
    * `workshop/_candidates.css` — candidats de review de tome et complétion manuelle par champ
    * `workshop/_logs.css` — console des journaux en direct et historique d'atelier
    * `workshop/_modals.css` — surcharges de la modale de review de tome et isolation `#manualReviewModal`
    * `workshop/_responsive.css` — points de rupture responsive (1100px, 860px, 520px) & `prefers-reduced-motion`
...
| `services/volume_enrichment/job.py` | Passe de bibliothèque en thread dédié. Superpose les overrides magiques de l'atelier après `resolve_index` |
| `services/workshop.py` | Payload atelier, URLs de jaquette Kavita, grains d'envoi, Magic, Review tome, reset |
| `services/kavita_cover_cache.py` | Cache disque des jaquettes Kavita déjà lues (`DATA_DIR/kavita_covers`) |
| `services/workshop_form.py` | Fiche metadata Kavita complète + extras tome ; envoi en force ; un seul POST metadata |
| `routes/volume_enrichment.py` | Aperçu / application / passe / statut / annulation, tous derrière une garde 403 |
| `routes/workshop.py` | API atelier + `GET /api/kavita-cover/...` (session Meta, jamais la clé API) |
...
L'envoi atelier est toujours en force : une valeur proposée écrase ce que Kavita détient déjà, verrous compris. Une valeur proposée vide n'est jamais écrite (y compris `localizedName`). La carte série est la fiche Kavita complète (année, statut, âge, langue, éditeur, liens, genres, tags, staff) et part en **un** `POST /api/Series/metadata`. Les champs vides restent visibles et ambre (`.workshop-field--empty`). Le staff en trop se range sous Plus (`details.workshop-more`), ouvert/fermé mémorisé dans `localStorage` (`workshop_show_more`). Les champs courts (année, statut, âge, langue, staff) partagent une grille à 6 colonnes (trois par ligne). Le Champ Magique est une ligne libellée sur chaque carte. Les jaquettes sont un livre 2:3 avec **Choisir** ; `startCoverSearch` pose un `data-cover-url` en attente et l'envoi l'écrit (`upload_series_cover` / jaquette de chapitre), pas `POST /update-cover`. Le titre de série est un lien vers `{KAVITA_UI_URL}/library/{libraryId}/series/{id}`. La barre des tomes est `position: sticky` dans `#workshopMain`. Le journal live (`#log-console`) est une troisième colonne ; `websocket.js` s'y branche déjà. `/` pose le focus sur la recherche du rail ; ← → se désactivent en bout de liste filtrée. **Manual Review** / **Super Review** de la fiche posent `window.WORKSHOP_FORCE_REVIEW` (cette page n'a pas les cases de la barre latérale, sinon `isReviewQueueModeOn()` resterait faux), ouvrent la modale d'attente via `mrPrepareForBatch()`, puis POST `/force-sync`. La file est bornée à cette série (même filtre que le Companion), pour qu'une file déjà en attente n'ouvre pas à la place de la série à l'écran. `sync_settled` appelle `mrOnSyncSettled` quand `settleSingleSync` est absent (`batch.js` n'est pas chargé ici). Confirmer depuis cette page envoie `workshop: true` ; `/confirm` remplit la fiche et une jaquette en attente, sans écrire Kavita (`apply_manual_review(..., workshop=True)`). La Review d'un tome remplit `#mrAboveList` avec `data-kind=volume` et force le panneau pick visible ; confirmer enregistre l'override et remplit la carte (`confirm_volume_review`), toujours sans écriture Kavita. La réutilisation de `#manualReviewModal` pour les tomes est strictement isolée de la revue de série (`isModalOpen()` ignore le mode volume, BF194) ; une navigation clavier dédiée aux tomes (Échap, Entrée, Flèches, 1–9, double-clic) est branchée dans `volumes.js`. Les candidats issus des requêtes ISBN et titre portent `provider` et `provider_ref`. L'auto-save de brouillon (`draft-series`) fusionne les `_external_ids` et jaquettes existants. Les tomes stagés (`_staged: True`) conservent leur état `data-dirty="1"` et les compteurs de barre au rechargement (F5) et à la navigation du rail ; les overrides séries restent dirty. Fermer la modale efface le flag. Le journal de gestes nomme le tome, pas `#chapter_id`. La passe automatique n'écrit toujours que `INDEX_FIELDS`. `SKIPPED` / `NOTHING_FOUND` répond HTTP 200 avec `noop`, pas 500. `_pass_blocks` n'est vrai que si la passe automatique est *sur cette série*. Un Reset de tome efface aussi la ligne `volume_unit_cache` de ce chapitre, pour que la passe ne le tienne pas pour clos. `send_selection` garde `claim_series_write` jusqu'au dernier tome et consigne `sent` et `total` dans l'historique. L'envoi unitaire nettoie les attributs de la carte via `markDoneClean([data])` et notifie `SupporterNag`. Des gardes défensives bloquent les requêtes orphelines quand `seriesId` est nul. Un rechargement après envoi conserve les cartes sales et les cases. `workshop_payload` porte `skipped_reason` (`oneshot` / `specials`) pour la bannière de l'atelier ; un override Magic/Review remplit `inscribed` pour que l'envoi écrive ce que le formulaire montre. Le rail partage les clés localStorage du tableau de bord ; un clic charge `GET /api/series/<id>/workshop` dans le volet (`pushState`, pas de rechargement). `GET /api/kavita-cover/...` sert `DATA_DIR/kavita_covers` et envoie Cache-Control. Tests : `tests/test_workshop.py`, `tests/test_volume_unit_cache.py`, `tests/test_workshop_audit_fixes.py`.
...
`matching.unmatchable_reason(units, series_name)` s'exécute dans les chemins fournisseur — `_enrich_one_series_locked` pour la passe, `build_series_plan` pour l'écriture d'une série — juste après la lecture des tomes et **avant** tout accès réseau, et dans `workshop_payload` pour la bannière (aucun réseau). Il rend `oneshot`, `specials` ou `""`.
...
**Le hub de scan Kavita est l'exception qui ne doit pas être une greenthread.** Flask-SocketIO possède déjà le WebSocket *serveur*. Un *client* SignalR sur une greenthread fige les Live Logs et le HTTP (même famille que `debug/repro_cover_eventlet.py`). `services/kavita_hub.py` tourne donc sur `eventlet.patcher.original("threading")` avec des sockets non patchées : JWT, negotiate et la boucle du hub restent hors du hub eventlet. Les callbacks ne poussent que `"scan"` dans `auto_sync_wake_queue` ; `get_all_series` et l'enqueue restent sur le worker Auto-sync existant. Ne jamais logger le JWT plugin. Tests : `tests/test_kavita_hub_parse.py`, `tests/test_kavita_hub_live.py`.

Chaque vague enfilée par l'Auto-sync écrit `auto_sync_runs` / `auto_sync_run_items` (C97). Le worker n'enregistre que les jobs `origin=auto` ; Stop passe les jobs auto encore en file à `stopped`. Clôturer une vague incrémente aussi les clés lifetime `auto_sync_*` une fois (C98). Le filtre tableau de bord `AUTO_SYNC` (C99) ne garde que ces `series_id`. Le bouton de la barre du haut est non-lu ou en cours, pas un récap de lot dashboard. Pas maille-série : le nettoyage des orphelines ne doit pas jeter le rapport. Tests : `tests/test_auto_sync_report.py`.
...
#### H. Les doublons de l'Inventaire sont un script, un comparatif volumétrique et un scan Kavita (C85, C110)

`DELETE /api/Series/{id}` n'enlève que la fiche. Les fichiers restent ; le scan suivant recrée la série. Meta ne monte pas les médias Kavita et ne déplace pas les fichiers sans le consentement explicite de l'utilisateur. Pour résoudre cela proprement :

* Meta génère des scripts de déplacement/suppression sécurisés en POSIX Bash (`.sh`) et en Windows PowerShell (`.ps1`) (`services/library_audit/dup_script.py`). Meta n'exécute jamais le script lui-même.
* Les groupes de doublons affichent les comptes réels de tomes et de chapitres pour chaque copie, calculent `recommended_keep_id` (la copie la plus fournie) et affichent un badge « 🌟 Recommandé (plus complet) ». L'interface oppose une confirmation explicite si l'utilisateur tente de jeter une copie possédant plus de tomes/chapitres que la copie conservée.
* Après avoir exécuté le script sur son hôte/NAS, l'utilisateur peut cliquer sur **⚡ Déclencher le scan Kavita** (`POST /api/libraries/<library_id>/kavita-scan`) directement depuis la modale des Doublons dans MetaKavita pour lancer le scanner de Kavita via `KavitaAPI.scan_library()` sans avoir à ouvrir l'interface de Kavita.
* Si le déplacement/suppression de fichiers laisse une coquille de série vide dans Kavita (0 volume), MetaKavita propose `POST /api/series/<id>/purge-empty` pour supprimer la coquille vide via `KavitaAPI.delete_series()`. Cette route vérifie défensivement `KavitaAPI.is_series_empty()` en amont (refusant toute série avec tomes ou chapitres existants), et déclenche une purge chirurgicale unitaire du cache (`purge_single_series_from_all_caches`) sans toucher aux autres séries.
* La suppression d'une seule série (purge-empty, SignalR `SeriesRemoved`) doit TOUJOURS appeler `purge_single_series_from_all_caches(series_id)` et non `clean_orphaned_cache()`, qui exige le set complet des identifiants actifs de Kavita.
* La normalisation des chemins (`normalize_inventory_folder_trash`) prend en charge aussi bien les chemins POSIX (`/...`) que Windows avec lettre de lecteur (`C:/...`, `D:\...`) pour les scripts PowerShell `.ps1`.
* Les doublons résolus peuvent être archivés via **Marquer comme traité** (`POST /api/libraries/<library_id>/duplicates/dismiss` avec `reason="resolved"`), et la mise à jour des dismissals est répercutée à la fois sur la bibliothèque ciblée et sur la vue globale `'all'`.
* Le nettoyage des caches orphelins (`clean_orphaned_cache`) s'exécute automatiquement en fin de scan d'inventaire, purgeant les séries supprimées de `series_cache`, `volume_report_cache`, et réajustant/supprimant les groupes réduits de `duplicate_group_cache`.
* Le hub SignalR (`services/kavita_hub.py`) écoute l'événement `SeriesRemoved` et purge chirurgicalement les caches locaux de cette série via `purge_single_series_from_all_caches`.
* Le préfixe est un chemin POSIX ou Windows (`INVENTORY_FOLDER_PATH_PREFIX`, `resolve_script_folder_path`), pas une URL HTTP.
* Tests : `tests/test_dup_folder_script.py`, `tests/test_library_audit.py`.
...
#### C. Review Manuelle en Atelier : Contrat de Staging & Déroulement des Étapes
- Lorsque la Review Manuelle ou Super Review est ouverte depuis l'atelier (`html.workshop-page`, `window.WORKSHOP_FORCE_REVIEW = true`) :
  - La revue déroule le cycle complet des étapes : sélection de candidat → choix de jaquette (si actif) → prévisualisation et ajustement des métadonnées (`phase = "edit"`).
  - Les préférences utilisateur sont injectées via `window.WORKSHOP_CONFIG` (respectant `MANUAL_REVIEW_EDIT` et `MANUAL_REVIEW_COVER_PICK`).
  - À la confirmation explicite de l'utilisateur (`POST /api/manual-reviews/<id>/confirm` avec `workshop: true`) :
    - Les champs modifiés et la jaquette choisie sont appliqués en brouillon sur la fiche série (`save_workshop_series_override` → `workshopApplyReview`).
    - La review est close en statut `PENDING` (pas `COMPLETED`) tant que l'utilisateur ne clique pas sur « Envoyer la fiche ».
    - **Aucune écriture Kavita n'a lieu** lors du staging de review (`update_series_metadata` et `upload_series_cover` ne sont jamais appelés).
...
- *Déballage défensif des tuples de l'API Kavita* : `send_series` inspecte le retour de `update_series_general` et `upload_series_cover` de façon générique comme une séquence, évitant tout `ValueError: too many values to unpack` lors de retours en 3-tuple `(ok, detail, sealed)` ou 2-tuple.
- *Contraste des listes déroulantes select en mode sombre* : Les menus natifs de Chromium/Windows retombent sur un fond blanc si `color-scheme: dark` n'est pas spécifié. La déclaration de `color-scheme` et de styles opaques sur `option`/`optgroup` (`#0f172a` en atelier, `#12151f` sur le tableau de bord) prévient tout texte clair illisible sur fond blanc dans les champs vides (ambre) ou normaux.
- *Compteur de champs complétés dans Plus de champs* : Le `<summary>` dépliable des champs secondaires affiche le ratio de complétion en temps réel : `Plus de champs (x complétés / y)` (EN : `More fields (x filled / y)`), réactif en direct à la frappe, aux menus déroulants et aux injections de revue manuelle.

#### F. Audit complet & fiabilisation de l'Atelier (BF192)
- *Modale de review de tome* : `#manualReviewModal[data-kind="volume"]` affiche un en-tête de contexte dédié (`#mrVolumeContext`) avec le nom de la série, le numéro et le titre du tome, tout en masquant les contrôles propres aux séries (`.mr-series-block`, `#mrQueueBadge`). Les candidats affichent une miniature de jaquette, les métadonnées sans séparateur orphelin, et pré-sélectionnent le premier candidat.
- *Cycle de vie de l'état dirty* : `syncDirtyState()` calcule dynamiquement l'état modifié. Les envois réussis (`workshopSendSeries`, `send_volume`, `markDoneClean`) retirent proprement `data-dirty`, `data-cover-url`, `data-cover-display` et mettent à jour `data-initial`. `snapshotDirty()` ne capture la fiche série et les jaquettes de tomes que si la carte est réellement dirty, éliminant toute résurrection de brouillon après envoi.
- *Isolation du timer de brouillon* : `scheduleSeriesDraft()` capture `targetSid = seriesId` et vérifie `seriesId === targetSid` à l'exécution. Toute navigation (`goTo`, `loadSeries`) annule le timer en vol, empêchant les données de la série A d'écraser le brouillon de la série B en base SQLite.
- *Assainissement de la date sentinelle* : `inscribed_from_chapter` et `volumeFieldsOf` détectent l'horodatage sentinelle de Kavita (`0001-01-01T00:00:00`) et le vident (`""`), restaurant la surbrillance ambrée de champ vide.
- *Prise en compte de l'ISBN manuel* : `openVolumeReview` extrait l'ISBN saisi dans l'input et l'envoie à `POST /api/series/<id>/workshop/review`, qui le transmet à `begin_volume_review` et `fetch_by_isbn`.
...
- *Préservation de la barre de santé & des compteurs* : `applyDuplicateFlagsToUi` et `_updateHygieneCounts` fusionnent les compteurs partiels et protègent `_updateHygieneHealth` contre l'absence des champs `series`/`healthy`, empêchant la barre de santé du tableau de bord de disparaître lors de l'ouverture des doublons ou du dismiss d'un groupe.
- *Synchronisation des doublons ignorés & partage global* : `library_duplicates_dismiss` met à jour `hygiene_library_meta` en SQLite avec le décompte décrémenté. `list_dismissed_group_keys` et `list_dup_dismissals` interrogent les dismissals sur l'ID de bibliothèque et sur `'all'`, permettant aux exclusions de persister sans déconnexion entre vue bibliothèque et vue globale.
- *Correctifs de scraping ComicVine & AniList* : `_comicvine_issues` et `series_identity.py` assainissent tout préfixe `4050-` ou `4050` avant de forger l'URL. `resolve_catalog_expected` teste `not ids.get("anilist")` au lieu de `not ids`, autorisant la recherche par titre AniList lorsqu'une série dispose d'un identifiant MAL ou MangaBaka sans identifiant AniList.
- *Empilement des modales & ergonomie* : La touche Échap ferme les modales dans l'ordre de priorité (`volumeReportModal` -> `duplicatesModal` -> `missingVolumesModal`), ne refermant que la modale active du dessus. Les lignes du tableau des manquants portent `data-series-name` pour éviter de polluer le titre avec `#ID`, et les exports CSV/TXT sur bibliothèque non analysée renvoient des fichiers vides avec en-têtes valides plutôt qu'un téléchargement d'erreur JSON 404. La route `/audit-badges` enrichie transmet `completion_state`, `forced` et `unit` pour restituer immédiatement les couleurs des cartouches.

#### I. Fiabilisation du cycle de vie des tomes et de l'Atelier (C111, BF197)
- *Nettoyage de la jaquette lors de l'envoi* : Lorsque `send_volume` écrit avec succès dans Kavita (`status == "DONE"`), la clé `cover_url` et le drapeau `_staged` sont purgés de `clean_payload` avant enregistrement dans `volume_unit_overrides`. Cela élimine la résurrection des jaquettes au rechargement (F5) et empêche les ré-envois inutiles de jaquettes.
- *Calcul dirty de la série basé sur `initial_value`* : `seriesBaseline()` compare les valeurs actuelles à `f.initial_value` (état brut Kavita) plutôt qu'à `f.value` (qui intègre déjà les overrides de SQLite), garantissant que `seriesIsDirty()` détecte fidèlement les modifications en attente.
- *Brouillons de tomes persistants (`POST /draft-volume`)* : Toute saisie sur une carte de tome déclenche un appel debouncé (500 ms) vers `/api/series/<id>/workshop/draft-volume`, enregistrant le brouillon en SQLite avec `_staged: True` et `_source: manual`. La passe automatique de bibliothèque (`_enrich_one_series`) vérifie `_staged` et ignore ces tomes, empêchant tout écrasement du travail en cours.
- *Collecte multi-fournisseurs (`all_scrapers=True`)* : `fetch_by_isbn` et `fetch_by_title_volume` intègrent le paramètre `all_scrapers: bool = False`. Lors d'une review de tome en Atelier, `begin_volume_review` passe `all_scrapers=True` pour interroger l'ensemble des scrapers actifs sans s'interrompre au premier résultat, renvoyant tous les candidats indexés sous `{key}_{scraper.id}`.
- *Déblocage de la reprise de passe automatique au reset unitaire* : Le reset d'un seul tome via `clear_volume_unit_states(series_id, chapter_id)` supprime à la fois la ligne du chapitre et la sentinelle de série `SERIES_PASS_CHAPTER_ID` (`chapter_id = 0`). Cela permet à `list_enriched_series_ids()` de ne plus considérer la série comme achevée, autorisant une passe de reprise (`resume=True`) à retraiter le tome réinitialisé.
- *Purge systématique du cache d'hygiène* : Tous les points d'entrée d'envoi de l'Atelier (`send_volume`, `send_selection`, `send_series`) appellent immédiatement `purge_series_hygiene_cache(series_id, keep_overrides=True)`, maintenant les cartouches d'inventaire et les rapports de volumes à jour.
- *Normalisation des dates et toasts réseau* : Les horodatages ISO (`YYYY-MM-DDTHH:MM:SS`) sont tronqués en `YYYY-MM-DD` et la sentinelle `0001-01-01` est ignorée. Les blocs catch des boutons d'envoi collectif affichent `toast(T().err_network)`.

#### J. Refonte CSS moderne et haute densité de l'Atelier (C112)
- *Densité d'information doublée & jaquettes valorisées* : Mise en page compacte réduisant l'encombrement vertical tout en offrant aux couvertures une place moderne et généreuse. La fiche série voit sa hauteur divisée par deux (~700 px à ~380–420 px) avec une jaquette livre valorisée de 124×186 px et des inputs de 26–28 px. Les cartes de tomes sont allégées de ~450 px à ~190–210 px avec de superbes jaquettes de 104×156 px (ratio 2:3) qui épousent toute la hauteur du bloc de métadonnées sans espace vide. La case à cocher s'aligne proprement contre la jaquette, le Champ Magique forme un bandeau compact, les inputs passent à 24 px et le bouton « Choisir » affiche son intitulé. L'utilisateur peut ainsi apprécier et comparer 2 à 3 tomes détaillés simultanément sans défilement perpétuel.
- *Halo ambré doux pour les champs vides* : Remplacement des bordures jaunes/oranges épaisses et agressives (`rgba(251, 191, 36, 0.75)` + ombres intérieures) par une teinte ambrée chaleureuse et subtile (`rgba(251, 191, 36, 0.38)` en bordure, `rgba(251, 191, 36, 0.03)` en fond). Le survol et le focus apportent un halo ambré délicat sans ressembler à des erreurs de validation critiques.
- *Dark Glassmorphism moderne & finitions* : Abandon du marron/orange terne au profit d'un fond ardoise sombre moderne (`rgba(17, 24, 39, 0.75)` / `rgba(13, 20, 32, 0.85)`), paré d'un flou d'arrière-plan (12–16 px) et de bordures fines 1px (sarcelle pour la série, ambre doux pour les tomes). La barre d'action sticky (`#workshopBar`) flotte proprement avec boutons groupés et bouton principal en dégradé orange. Le rail gauche gagne +15% de séries visibles grâce à des fiches de 48 px. Le support du mode clair (`[data-theme="light"]`) est intégralement préservé.
- *Alignement du scroll virtuel et robustesse des raccourcis JS* : La constante de scroll virtuel du rail `ROW_H` est calée sur 48 (correspondant à la hauteur de 48 px dans `volumes.css`), éliminant tout décalage lors du défilement des grandes bibliothèques. `isTypingTarget` exclut les éléments non textuels lorsque `kind === 'volume'`, rétablissant les raccourcis clavier dans la revue de tome (Entrée, Flèches Haut/Bas, touches 1–9). Les jaquettes choisies pour un tome déclenchent automatiquement `scheduleVolumeDraft` pour persister en base SQLite. Des gardes strictes `display: none !important;` empêchent l'affichage accidentel des balises `[hidden]` (jaquettes en erreur ou absentes) et le responsive préserve la grille `18px 58px 1fr` jusqu'à 520 px.

#### K. Duplication série -> tomes et modernisation inspirée de la Revue Manuelle (C113)
- *Contrôles de formulaire inspirés de la Revue Manuelle* : Refonte des inputs et selects de l'Atelier sur le modèle élégant de la Revue Manuelle et de l'écran « Ajuster avant envoi ». Remplacement du fond noir profond par un fond ardoise sombre moderne (`#141926`), des coins arrondis à 7–8 px, une hauteur confortable de 30 px (`padding: 4px 10px`) et une bordure subtile (`rgba(148, 163, 184, 0.2)`). Les libellés abandonnent les majuscules rigides au profit d'une casse naturelle (`text-transform: none`, taille 11 px, graisse 600) avec icônes de verrouillage épurées. Les champs vides suppriment tout liseré jaune criard au profit d'une teinte ambrée très douce et non agressive (`rgba(251, 191, 36, 0.26)` en bordure, `0.02` en fond).
- *Duplication unitaire en 1 clic (`.workshop-cascade-btn`)* : Tous les champs compatibles entre série et tomes (`writers`, `pencillers`, `coverArtists`, `translators`, `genres`, `tags`, `ageRating`, `language`, `webLinks`, `summary`) intègrent un micro-bouton élégant dans l'en-tête de leur libellé. Un clic lit la valeur courante de la série et la propage instantanément à l'ensemble des cartes de tomes chargées, mettant à jour l'input, marquant les tomes en `data-dirty="1"`, planifiant la sauvegarde automatique en brouillon SQLite (`scheduleVolumeDraft`) et confirmant l'action via un toast d'application (`workshop_cascade_success`).
- *Action globale « Dupliquer vers les tomes » (`#workshopCascadeAllBtn`)* : Un bouton d'action paré d'une icône SVG dédiée (`#mk-ico-cascade`) dans la barre d'actions de la fiche série permet de cascader en 1 clic l'ensemble des métadonnées communes renseignées vers tous les tomes, avec marquage dirty et persistance automatique.
- *Puces de statut modernes pour les tomes* : L'en-tête de chaque carte de tome affiche désormais une puce de statut moderne (`.workshop-status-chip`) calquée sur `.mr-chip` avec accents thématiques pour `DONE` (vert), `STAGED` (sarcelle) et `PENDING` (ardoise), offrant une visibilité immédiate sur le cycle de vie de chaque unité.
- *Intégration fluide de la couverture de série (Zéro vide)* : La fiche série utilise une grille CSS continue à 7 colonnes (`136px repeat(6, minmax(0, 1fr))`) avec `display: contents;` sur `#workshopSeriesFields`. La jaquette livre 136×204 px s'étend sur les rangées 1 à 3 aux côtés du titre, du nom alternatif et du résumé. Les métadonnées inférieures (année, statut, classification, éditeur, genres, tags, auteurs, accordéon Plus et boutons d'action) s'étendent en pleine largeur directement sous la jaquette, éliminant tout vide ou espace mort.
- *Intégration fluide de la couverture de tome (Zéro vide)* : Les cartes de tomes utilisent une grille continue à 8 colonnes (`20px 104px repeat(6, minmax(0, 1fr))`) avec `display: contents;` sur `.workshop-volume-meta` et les champs primaires `.workshop-volume-fields`. La jaquette tome 104×156 px et la case à cocher s'étendent sur les rangées 1 à 4 aux côtés de l'en-tête, du Champ Magique, du titre, de l'ISBN et de la date de parution. Les éléments inférieurs (résumé en textarea, accordéon Plus de champs et boutons d'action) s'étendent en pleine largeur (`1 / -1`) directement sous la jaquette, éliminant tout vide ou espace mort.
- *Journalisation WebSocket en temps réel lors des envois* : L'écriture dans Kavita via `send_volume` (envoi unitaire), `send_selection` (envoi groupé ou de tous les tomes) et `send_series` (fiche série) émet des logs `logging.info(...)` interceptés par `WebSocketLogHandler` et parsés par `websocket.js`. Chaque log intègre le label de série `[« Nom » (id)]`, le marqueur vert `✅`, le libellé de l'unité (ex. `tome 2`) et les champs écrits, offrant un retour visuel en temps réel dans la console de logs de l'Atelier.
