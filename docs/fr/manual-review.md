...
**Super Manual Review** interroge tous les scrapers utilisables (pas seulement les trois slots). Après le provider #1 (ISBN / auteurs), les autres partent en parallèle — toujours une série à la fois, et le timer de chaque scraper s'applique encore. **Cover picker** permet de choisir une couverture avant confirm. **Review sounds** joue des sons UI. **Purge queue** vide les séries garées.
...
Si **Éditer avant confirmation** est allumé (`MANUAL_REVIEW_EDIT`), **Ajuster avant envoi** est le dernier récap sur le tableau de bord : URL de couverture, titre, titre localisé, année, statut, classification d'âge, format, éditeur, résumé, genres, tags, staff — avec la barre Master / Fusion. Chaque champ écrivable a une case : cochée = écriture, décochée = Kavita inchangé. Titre et format sont en lecture seule. Un override **champs ciblés** qui a retiré un champ verrouille cette case. Pied : **Retour**, **Passer**, **Confirmer** (`Entrée` / `Échap` / `Retour arrière`). *(Note : Dans l'[Atelier](volumes.md), cette fenêtre intermédiaire n'apparaît pas ; le choix ou la complétion manuelle s'applique directement sur la fiche série de l'atelier, qui sert elle-même de surface d'édition).*
...
Réglages : `MANUAL_REVIEW_MODE`, `MANUAL_REVIEW_EDIT`, `MANUAL_REVIEW_SUPER`, `MANUAL_REVIEW_SOUNDS` — voir [Configuration](configuration.md). Le [mapping par champ](dashboard.md) de l'Auto ne s'applique pas ici.
