...
- *Modern Status Chips & Volume Badging*: Volume card headers feature modern status pills (`.workshop-status-chip`) inspired by `.mr-chip` with dedicated color accents for `DONE` (green), `STAGED` (teal), and `PENDING` (slate), providing immediate at-a-glance visibility into the lifecycle of every volume.
