...
**Manga.** Manga-News lists the French volumes of a series (album title, summary, ISBN, release date), one page per volume, and is asked first on a manga library — last resort on Comic (Flexible). MangaDex still fills in the real cover of every volume, one request for the whole series, in your reading language when that edition exists; those covers do not cancel the rest of the cascade. Volumes that already carry an ISBN in Kavita are looked up on Google Books, then Open Library, then Hardcover. For volumes with no ISBN at all and no Manga-News hit, `VOLUME_ENRICH_EXPERIMENTAL` searches Google Books by series title plus volume number. It stays a search: each candidate's title must contain the series name *and* announce the volume number before anything is written, and you should read the preview before applying.

#### 11. Library Inventory (v1.7.0, `LIBRARY_INVENTORY_ENABLED`)
An **Inventory** panel above the series list tells you which series are incomplete. **Analyze library** runs in the background: it counts the volumes (or chapters) you own in Kavita, asks the provider cascade how many there should be, and clusters look-alike series. You get a health bar, **Missing / Duplicates / No id** chips, an `N/M` badge on each row coloured by completion, a per-series report with the missing numbers folded into ranges, CSV / TXT exports, and duplicate groups you can dismiss. Each duplicate row shows Kavita's folder path: copy it, tick the extra copies (at least one series per group stays unticked), and copy or download a bash script (`mv` into `INVENTORY_FOLDER_TRASH`, or `rm`). `INVENTORY_FOLDER_PATH_PREFIX` (set in the Duplicates modal) is glued in front of each Kavita path in that script — e.g. `/mnt/media` + `/comics/…` → `/mnt/media/comics/…`. Meta never runs it and never deletes Kavita records, because a delete that leaves the files on disk comes back on the next scan. Expected counts can be forced by hand, and a series no catalogue will ever know can be excluded from the counters while still being scraped. Inventory only reads Kavita — it never writes volume metadata and never merges series. Switch it off from the sidebar (**Inventory** category) and the panel, badges and API all go away.
...
**Manage + Store (v1.6.3 / C61, core sync C62 in v1.6.4):** Help → **Manage scrapers** (`/manage-scrapers`) and **Community Store** (`/scraper-store`). The registry loads only from `data/scrapers/` (on boot, `sync_core_scrapers()` refreshes `is_core` modules from the community catalog, with Docker-image fallback if GitHub is unreachable; `AUTO_UPDATE_CORE_SCRAPERS` default on). Core scrapers can be disabled (`DISABLED_SCRAPERS`); community ones install/update/delete from the Store (sha256-checked GitHub catalog) with hot reload — no container restart for Magasin installs. Manual drop-ins still need a restart (or a later store/reload action).
...
*   **Purge du contexte (Force Update + « Effacer le contexte »)** : en mise à jour forcée, ignore le contexte de matching déjà présent dans Kavita (auteurs / éditeur / année / genres — l’ISBN reste disponible pour OpenLibrary etc.) pour briser les boucles de faux positifs. Si le champ **WebLinks** est ciblé, les liens web Kavita sont **remplacés** par ceux du scrape (plus d’empilement des anciennes URLs erronées).
...
**Mangas.** Manga-News liste les tomes VF d'une série (titre d'album, résumé, ISBN, date de parution), une page par tome, et passe devant sur une bibliothèque manga — en dernier recours sur une Comic (Flexible). MangaDex comble encore le manque avec la vraie couverture de chaque tome — un appel pour toute la série, dans votre langue de lecture quand cette édition existe — et ces couvertures n'annulent pas le reste de la cascade. Les tomes qui portent déjà un ISBN dans Kavita sont cherchés sur Google Books, puis Open Library, puis Hardcover. Pour les tomes sans ISBN et sans fiche Manga-News, `VOLUME_ENRICH_EXPERIMENTAL` cherche sur Google Books par titre de série et numéro de tome. Cela reste une recherche : le titre de chaque candidat doit contenir le nom de la série *et* annoncer le numéro du tome avant que quoi que ce soit ne s'écrive, et mieux vaut relire l'aperçu avant d'appliquer.

**Fournisseur imposé** (`VOLUME_PROVIDER`, catégorie **Tomes et albums**). Le menu distingue deux familles, parce qu'elles ne rendent pas la même chose. Ceux qui *listent les albums d'une série* les ramènent tous en un appel, titres et résumés compris : c'est le chemin de la BD et des comics, et de Manga-News pour les tomes VF. Ceux qui *identifient un tome par son ISBN* — Google Books, Open Library, Hardcover — travaillent tome par tome à partir de l'ISBN que Kavita détient : ils complètent un index incomplet, et un tome sans ISBN ne leur dit rien. Imposer l'un d'eux supprime la liste d'albums au lieu de la demander à quelqu'un d'autre, et n'interroge que lui ; quand il n'a aucune prise, le journal le dit et désigne le réglage.

#### 11. Inventaire de la bibliothèque (v1.7.0, `LIBRARY_INVENTORY_ENABLED`)
Un panneau **Inventaire**, au-dessus de la liste des séries, vous dit lesquelles sont incomplètes. **Analyser la bibliothèque** travaille en arrière-plan : il compte les tomes (ou les chapitres) que vous possédez dans Kavita, demande à la cascade de providers combien il devrait y en avoir, et regroupe les séries qui se ressemblent. Vous obtenez une barre de santé, des chips **Manquants / Doublons / Sans id**, une cartouche `N/M` colorée selon la complétion sur chaque ligne, un rapport par série avec les numéros manquants pliés en intervalles, des exports CSV / TXT, et des groupes de doublons à ignorer. Chaque ligne de doublon montre le dossier Kavita : à copier, à cocher (une série au moins reste décochée par groupe), puis à emporter dans un script bash (`mv` vers `INVENTORY_FOLDER_TRASH`, ou `rm`). `INVENTORY_FOLDER_PATH_PREFIX` (dans la modale Doublons) est collé devant chaque chemin Kavita dans ce script — ex. `/mnt/media` + `/comics/…` → `/mnt/media/comics/…`. Meta ne l'exécute jamais et ne supprime plus de fiche Kavita, parce qu'un delete qui laisse les fichiers sur le disque revient au scan suivant. L'attendu peut être forcé à la main, et une série qu'aucun catalogue ne connaîtra jamais peut être exclue des compteurs tout en restant scrapée. L'Inventaire ne fait que lire Kavita : il n'écrit aucune métadonnée de volume et ne fusionne jamais de séries. Désactivez-le depuis la sidebar (catégorie **Inventaire**) et le panneau, les cartouches et l'API disparaissent.
...
**Manage + Magasin (v1.6.3 / C61, sync core C62 en v1.6.4) :** Aide → **Gérer les scrapers** (`/manage-scrapers`) et **Magasin** (`/scraper-store`). Le registre ne charge que `data/scrapers/` (au boot, `sync_core_scrapers()` rafraîchit les modules `is_core` depuis le catalogue community, fallback image Docker si GitHub injoignable ; `AUTO_UPDATE_CORE_SCRAPERS` défaut on). Les scrapers core se désactivent (`DISABLED_SCRAPERS`) ; les community s’installent / se mettent à jour / se suppriment depuis le Magasin (catalogue GitHub + sha256) avec rechargement à chaud — pas de restart pour une install Magasin. Un dépôt manuel de fichier nécessite encore un restart.
