...
- [ ] **Volume / issue metadata (community ask #27):** Per-volume / comic-issue enrichment — `BaseScraper.scopes` already reserves `volume`; pipeline + UI still open. **Precursor shipped (Inventory / ex-hygiène):** analyze job, chips, badges `N/M`, cascade+backup expected, publication status, forced expected, missing export, dup scoring + dismiss/delete — no volume scrape write yet.
...
- [ ] **Métadonnées volume / issue (demande #27) :** enrichissement par tome / album — `scopes` volume déjà réservé sur `BaseScraper` ; pipeline + UI encore ouverts. **Précurseur livré (Inventaire / ex-hygiène) :** job Analyser, chips, badges `N/M`, attendu cascade+secours, statut publication, attendu forcé, export manquants, scoring doublons + dismiss/delete — pas encore d’écriture scrape volume.
...
- [x] **BF105. MR : masquer sous le seuil pendant le stream (v1.6.6) :** case persistante « Afficher sous le seuil » ; plus de flash weak mid-stream ; filet empty-above en fin de scrape.
- [x] **BF106. Un sync n'efface plus une couverture manuelle (v1.6.6) :** `coverImageLocked` (+ `dontMatch`) réinjectés depuis l'état Kavita courant dans les deux passes ; une clé absente était désérialisée à `false` et Kavita réinitialisait la couverture.
- [x] **BF107. La Super Review Companion propose le cover picker (v1.6.6) :** `/companion/embed` expose les options serveur (`COMPANION_EMBED.options`) ; les previews de couverture de l'embed portent `embed_token`.
- [x] **BF114. Chasse aux bugs après le check-up (v1.6.6) :** `_worker` borne son corps de boucle (`_abandon_sync_item` fait quand même avancer la barre du lot) — une exception inattendue tuait l'unique worker de sync jusqu'au redémarrage ; `_auto_sync_worker` survit à une config illisible ; nouvel événement `sync_settled` qui rend le bouton de la ligne quelle que soit l'issue (`series_status` n'est pas diffusé sur tous les chemins, et `PENDING_REVIEW` n'est pas un échec) ; `put_front(replace_pending=False)` pour `/force-sync`, pour qu'une file C63 en pause garde ses lignes ; `library_audit.js` fait passer ses 22 appels d'API et ses exports par `getRootPath()` (tout l'inventaire répondait 404 derrière un proxy en sous-chemin) ; un scan `catalog=false` réutilise le catalogue en cache au lieu de réécrire chaque série en *attendu inconnu* ; les verdicts de doublon sur id partagé sont déterministes et privilégient la concordance ; la progression d'inventaire émet via `extensions` au lieu d'importer `app` depuis un thread. Tests : `tests/test_sync_worker_resilience.py`, `tests/test_frontend_root_path.py`.
- [x] **BF108–BF113. Durcissement suite check-up du code (v1.6.6) :** la purge des orphelines exige un inventaire Kavita lu en entier (`KavitaAPI.last_inventory_complete`, un inventaire vide ne purge jamais) ; `/force-sync` enfile en tête de `sync_queue` (202) au lieu d'enrichir dans la requête, le bouton de la ligne se résout sur `sync_settled` ; la recherche de couvertures partage la cadence providers avec l'enrichissement (`services/provider_throttle.py`) ; les jetons d'embed Companion sont bornés à leur série sur toutes les routes `manual_review` et sur Socket.IO, opérations de masse refusées ; `auth_manager` ouvre SQLite via `db_manager._connect` (WAL + `busy_timeout`) ; un seul helper d'échappement HTML (`escapeHtmlText`, apostrophe incluse) et les cinq clés `AppTranslations` manquantes injectées. CI : job `ruff` + publication Docker conditionnée au workflow de tests réutilisable.
- [x] **Companion 1.0.23–1.0.26 (v1.6.6) :** chemins zip POSIX ; activation reverse proxy même hôte ([#34](https://github.com/raukorim-bot/MetaKavita/issues/34)) ; Champ Magique pour les covers ; previews de couverture proxifiées récupérées par le service worker pour survivre à Kavita HTTPS + Meta HTTP (contenu mixte) ; Super Review en fenêtre popup au lieu d'une iframe bloquée.
- [x] **C67. Reprise du design des modales (v1.6.6) :** modales d'audit passées sur le socle partagé (actions en pied, une seule zone de défilement, bouton de fermeture stylé) ; variantes de largeur `--modal-w` dont une variante 1180 px pour les tableaux ; tableaux d'audit repliés en cartes étiquetées sous 720 px au lieu de défiler sur le côté ; doublons refaits en lignes alignées avec en-têtes en pastilles et bandeau de seuil ; cover picker élargi à 920 px ; cartes de rapport et tuiles de récap alignées en flex ; sprite SVG inline pour les titres de modales et états vides / chargement / erreur illustrés.
- [x] **C66. Inventaire bibliothèque présentable + optionnel (v1.6.6) :** panneau pleine largeur avec barre de santé, fraîcheur d'analyse, analyse rapide (incrémentale, cache réutilisé seulement si le compte local correspond toujours) et annulation ; code couleur de complétion + liseré d'attendu forcé ; séries en chapitres comptées en chapitres ; numéros hors plage mis de côté au lieu de gonfler les comptes et d'inventer des trous ; intervalles au lieu de longues listes ; modale de rapport avec cartes de stats, unités en lazy et exclusion par série ; exports CSV / TXT conscients de l'unité ; seuil de doublons déplacé dans sa modale ; `LIBRARY_INVENTORY_ENABLED` masque la fonctionnalité et met ses routes en 403.
- [x] **C65. Provenance couverture manuelle + cartouche 🔒 (v1.6.6) :** protection déplacée du décochage de `cover` dans `targeted_fields` vers une colonne `cover_manual` dédiée ; cartouche cliquable sur la ligne de série ; interrupteur sidebar `COVER_FORCE_OVERWRITE` qui la lève pour tout un run (plus de clic série par série) ; un choix explicite gagne toujours.
