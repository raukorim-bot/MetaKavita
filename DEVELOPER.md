...
*   **`services/scraper_diagnostics.py`**: `/diagnostics` preflight + probes. C95: `probe_all` / the page load submit every target together (`ThreadPoolExecutor`, one worker per provider). `throttle_provider` still serializes two calls to the *same* scraper. Stream `POST /api/scrapers/probe-all?stream=1` announces every `start_scraper` before the first HTTP, then yields results as they finish. “Test cascade” remains `scope=active`.
...
*   **`services/scraper_diagnostics.py`** : préflight + probes de `/diagnostics`. C95 : `probe_all` / le chargement de la page soumettent toutes les cibles ensemble (`ThreadPoolExecutor`, un worker par fournisseur). `throttle_provider` sérialise toujours deux appels au *même* scraper. Le flux `POST /api/scrapers/probe-all?stream=1` annonce chaque `start_scraper` avant le premier HTTP, puis rend les résultats dans l'ordre de finition. « Tester la cascade » reste `scope=active`.
